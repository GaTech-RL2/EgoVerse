...
echo "== SMOKE: bf_nopre_win (EMA + GN + episode-scope crop) =="
srun -A hoffman-lab -p hoffman-lab --gres=gpu:a40:1 --cpus-per-task=8 --mem=64G -t 00:45:00 \
  ...
  model=bf_nopre_win trainer=ddp_pi description=smoke_win \
  \
...
for SPEC in "bf_nopre_win"; do
  J=$(MODEL=$SPEC NAME=$SPEC sbatch --parsable -J "$SPEC" bf_prdec_abl.sbatch)

#!/bin/bash
# Dispatcher for base_pose-augmented hierarchical training on push_cart datasets.
#
# Per (dataset x variant):
#   1. Step 2b: build <dataset>_basepose by adding obs.base_pose_integrated
#      (skipped if already present).
#   2. Train using the push_cart_p1_april6d_basepose Hydra config.
#
...
#
# Usage:
#   ./sew_basepose_batch_workflow.sh [--dry-run] [--monitor]
#                                    [--datasets ds1,ds2] [--variants v1,v2]
#                                    [--add-variant NAME OVERRIDES]
#
# Naming:
#   logs/<dataset>_basepose/<variant>/  (Hydra hydra.run.dir interpolation)

set -e
cd /coc/flash7/zhenyang/EgoVerse

TRAIN_CONFIG="experiments/hierarchical/push_cart_p1_april6d_basepose"
...
# Default null variant uses the config's stem defaults (noise_std=0.10, drop_p=0.20).
VARIANTS=(
    "null|"
)
...
for DATASET_NAME in "${DATASETS[@]}"; do
    ...
    for V_ROW in "${VARIANTS[@]}"; do
        ...
        JOB_TAG="sewbp_${DATASET_NAME}_${VARIANT_NAME}"
        ...
        SUBMIT_OUT=$(sbatch --parsable --job-name="${JOB_TAG}" \
            --export=ALL,DATASET_NAME="${DATASET_NAME}",TRAIN_CONFIG="${TRAIN_CONFIG}",DESCRIPTION="${VARIANT_NAME}",EXTRA_HYDRA_OVERRIDES="${EXTRA_OVERRIDES}" \
            submit_sew_basepose_training.sbatch)
        JOB_IDS+=("${SUBMIT_OUT}")

...
echo "== SMOKE: bf_prdec_hb (hard-band reg, ratio off) =="
srun -A hoffman-lab -p hoffman-lab --gres=gpu:a40:1 --cpus-per-task=8 --mem=64G -t 00:45:00 \
  ...
  model=bf_prdec_hb trainer=ddp_pi description=smoke_hb \
...
C=$(ls -t logs/indomain_c4/smoke_hb_*/csv_logs/lightning_logs/version_0/metrics.csv | head -1)
head -1 "$C" | tr "," "\n" | grep -c hb_rate && echo HB_METRICS_PRESENT
for M in bf_prdec_hb bf_symw1_hb; do

#!/bin/bash
#SBATCH --job-name=T_cup_BC2ID_DEBUG
#SBATCH --account=cvg-prof-m-2
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug/slurm-cup-BC2ID-debug-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug/slurm-cup-BC2ID-debug-%j.err
...
export HYDRA_RUN_DIR=${HYDRA_RUN_DIR:-/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/DEBUG/eve_aria_cup_cotrain_BC+2ID/${SLURM_JOB_ID}}

mkdir -p "${HYDRA_RUN_DIR}" /iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug

echo "[cup BC+2ID DEBUG] full indomain aria"
...
python ${EGOMAIN}/egomimic/trainHydra.py \
    ...
    name=eve_aria_cup_cotrain_BC2ID_debug_'"${SLURM_JOB_ID}"' \
    description=eve_aria_cup_cotrain_BC2ID_debug_'"${SLURM_JOB_ID}"' \

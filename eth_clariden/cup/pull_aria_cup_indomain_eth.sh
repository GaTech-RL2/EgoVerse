#!/bin/bash
#SBATCH --job-name=pull_aria_cup_indomain_eth
#SBATCH --account=cvg-prof-m-2
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/aria-pull-indomain-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/aria-pull-indomain-%j.err
...
# Pulls aria episodes for cup_on_saucer indomain (lab=eth). Used by both BC+1ID
# and BC+2ID variants (1ID adds valid_ratio override at training time).
...
DEST=/iopsstor/scratch/cscs/jiaqchen/data/EGOMIM/aria_cup_indomain_eth
mkdir -p "$DEST"
...
with open("/iopsstor/scratch/cscs/jiaqchen/aria_cup_indomain_eth_pick.csv", "w") as f:
...
done < /iopsstor/scratch/cscs/jiaqchen/aria_cup_indomain_eth_pick.csv

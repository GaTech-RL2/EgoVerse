#!/bin/bash
#SBATCH --job-name=T_cup_BC2ID2EVeth_DEBUG
#SBATCH --account=cvg-prof-m-2
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug/slurm-cup-BC2ID2EVeth-debug-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug/slurm-cup-BC2ID2EVeth-debug-%j.err
...
export ARIA_LOCAL_ZARR_DIR=${ARIA_LOCAL_ZARR_DIR:-/iopsstor/scratch/cscs/jiaqchen/data/EGOMIM/aria_cup_2id_2eveth}
export HYDRA_RUN_DIR=${HYDRA_RUN_DIR:-/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/DEBUG/eve_aria_cup_cotrain_BC+2ID+2EVeth/${SLURM_JOB_ID}}

mkdir -p "${HYDRA_RUN_DIR}" /iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug

echo "[cup BC+2ID+2EVeth DEBUG] indomain + everse_eth"
...
python ${EGOMAIN}/egomimic/trainHydra.py \
    ...
    name=eve_aria_cup_cotrain_BC2ID2EVeth_debug_'"${SLURM_JOB_ID}"' \
    description=eve_aria_cup_cotrain_BC2ID2EVeth_debug_'"${SLURM_JOB_ID}"' \

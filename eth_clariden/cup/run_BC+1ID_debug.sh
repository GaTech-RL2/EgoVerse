#!/bin/bash
#SBATCH --job-name=T_cup_BC1ID_DEBUG
#SBATCH --account=cvg-prof-m-2
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug/slurm-cup-BC1ID-debug-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug/slurm-cup-BC1ID-debug-%j.err
...
export HYDRA_RUN_DIR=${HYDRA_RUN_DIR:-/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/DEBUG/eve_aria_cup_cotrain_BC+1ID/${SLURM_JOB_ID}}

mkdir -p "${HYDRA_RUN_DIR}" /iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug

echo "============================================================"
echo "[cup BC+1ID DEBUG] half indomain aria (valid_ratio=0.6)"
...
python ${EGOMAIN}/egomimic/trainHydra.py \
    ...
    name=eve_aria_cup_cotrain_BC1ID_debug_'"${SLURM_JOB_ID}"' \
    description=eve_aria_cup_cotrain_BC1ID_debug_'"${SLURM_JOB_ID}"' \
    hydra.run.dir=${HYDRA_RUN_DIR} \
    trainer.num_nodes=1 \
    launch_params.gpus_per_node=1 \
    launch_params.nodes=1 \
    data.train_datasets.aria_bimanual.valid_ratio=0.6 \
    data.valid_datasets.aria_bimanual.valid_ratio=0.6

#!/bin/bash
#SBATCH --job-name=pull_aria_cup_2id_2eveth
#SBATCH --account=cvg-prof-m-2
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/aria-pull-2id-2eveth-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/aria-pull-2id-2eveth-%j.err
...
# Pulls Aria for BC+2ID+2EVeth: indomain (eth) + everse_eth.
...
DEST=/iopsstor/scratch/cscs/jiaqchen/data/EGOMIM/aria_cup_2id_2eveth
mkdir -p "$DEST"
...
with open("/iopsstor/scratch/cscs/jiaqchen/aria_cup_2id_2eveth_pick.csv", "w") as f:
...
done < /iopsstor/scratch/cscs/jiaqchen/aria_cup_2id_2eveth_pick.csv

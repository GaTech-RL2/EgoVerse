#!/bin/bash
#SBATCH --job-name=T_cup_BC0ID8EV_DEBUG
#SBATCH --account=cvg-prof-m-2
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug/slurm-cup-BC0ID8EV-debug-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug/slurm-cup-BC0ID8EV-debug-%j.err
...
export ARIA_LOCAL_ZARR_DIR=${ARIA_LOCAL_ZARR_DIR:-/iopsstor/scratch/cscs/jiaqchen/data/EGOMIM/aria_cup_8ev}
export HYDRA_RUN_DIR=${HYDRA_RUN_DIR:-/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/DEBUG/eve_aria_cup_cotrain_BC+0ID+8EV/${SLURM_JOB_ID}}

mkdir -p "${HYDRA_RUN_DIR}" /iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/debug

echo "[cup BC+0ID+8EV DEBUG] EgoVerse 4 labs aria"
...
python ${EGOMAIN}/egomimic/trainHydra.py \
    ...
    name=eve_aria_cup_cotrain_BC0ID8EV_debug_'"${SLURM_JOB_ID}"' \
    description=eve_aria_cup_cotrain_BC0ID8EV_debug_'"${SLURM_JOB_ID}"' \

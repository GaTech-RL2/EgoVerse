#!/bin/bash
#SBATCH --job-name=pull_aria_cup_8ev
#SBATCH --account=cvg-prof-m-2
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/aria-pull-8ev-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/cup/aria-pull-8ev-%j.err
...
# Pulls EgoVerse Aria for BC+0ID+8EV: 4 labs (rl2, song, wang, eth) all with
# task=cup_on_saucer.
...
DEST=/iopsstor/scratch/cscs/jiaqchen/data/EGOMIM/aria_cup_8ev
mkdir -p "$DEST"
...
with open("/iopsstor/scratch/cscs/jiaqchen/aria_cup_8ev_pick.csv", "w") as f:
...
done < /iopsstor/scratch/cscs/jiaqchen/aria_cup_8ev_pick.csv

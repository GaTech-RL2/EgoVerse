#!/bin/bash
#SBATCH --job-name=eve_bag_BC_4n4g_full
#SBATCH --account=cvg-prof-m-2
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/bag/full_4n/slurm-bag-BC-4n-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/bag/full_4n/slurm-bag-BC-4n-%j.err
...
############################################################
# Production 4-node x 4-GPU bag_groceries BC training (16 GPUs total).
# Effective batch = 32/GPU * 16 GPU = 512 (4x the single-node setup).
# LR scaled sqrt(4) = 2x: 3e-4 -> 6e-4 (model.optimizer.lr override).
# RUN_NAME / HYDRA_RUN_DIR / WANDB_RUN_ID are deterministic and stable across
# requeues so SLURM SIGUSR1 -> hpc_ckpt -> requeue picks up the same wandb run
...
export RUN_NAME="${RUN_NAME:-bag_BC_4n4g_full}"
...
export HYDRA_RUN_DIR=${HYDRA_RUN_DIR:-/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/FULL_4n/eve_bag_bc/${RUN_NAME}}
mkdir -p "${HYDRA_RUN_DIR}" "$(dirname /iopsstor/scratch/cscs/jiaqchen/egomim_out/main/bag/full_4n/.)"

echo "============================================================"
echo "[bag BC FULL 4n4g] EgoVerse-main 2000-epoch BC, 16 GPUs"

#!/bin/bash
#SBATCH --job-name=T_eve_aria_bag_cotrain_DEBUG
#SBATCH --account=cvg-prof-m-2
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/bag/slurm-cotrain-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/bag/slurm-cotrain-%j.err
...
export EVE_LOCAL_ZARR_DIR=${EVE_LOCAL_ZARR_DIR:-/iopsstor/scratch/cscs/jiaqchen/data/EGOMIM/bag_fixed_zarr}
export ARIA_LOCAL_ZARR_DIR=${ARIA_LOCAL_ZARR_DIR:-/iopsstor/scratch/cscs/jiaqchen/data/EGOMIM/aria_bag_zarr}
export HYDRA_RUN_DIR=${HYDRA_RUN_DIR:-/iopsstor/scratch/cscs/jiaqchen/egomim_out/main/DEBUG/eve_aria_bag_cotrain/${SLURM_JOB_ID}}

mkdir -p "${HYDRA_RUN_DIR}"

echo "============================================================"
echo "[run_cotrain_debug bag] EgoVerse-main bag cotrain debug pipeline"
...
python ${EGOMAIN}/egomimic/trainHydra.py \
    --config-name=train_zarr_eve_aria_bag_cotrain \
    name=eve_aria_bag_cotrain_'"${SLURM_JOB_ID}"' \
    description=eve_aria_bag_cotrain_'"${SLURM_JOB_ID}"' \

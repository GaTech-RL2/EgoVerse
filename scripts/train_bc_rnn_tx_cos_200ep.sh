...
NC3=/coc/flash7/paphiwetsa3/datasets/new_circle_3_first200
...
# the ONLY change is model=bc_rnn_pushshapes_paperexact_tx_cos, which swaps the
...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_tx_cos_200ep}
NAME=${NAME:-bcrnnTxCos200_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnTxCos200_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else

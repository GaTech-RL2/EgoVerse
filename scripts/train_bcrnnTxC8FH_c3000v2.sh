#!/bin/bash
#SBATCH --job-name=bcrnnTxC8FH_c3000v2
...
# bcrnnTxC8FH_c3000v2: FRESH full-history circle_3000 run. Cloned from the original
# new_circle_3 launcher recipe; ONLY the dataset folder_path + name changed.
# Model: bc_rnn_pushshapes_paperexact_tx_chunk8_fullhist; chunk_len=8; FH override rnn_horizon=baked.
...
SMOKE=${SMOKE:-0}
NAME=${NAME:-bcrnnTxC8FH_c3000v2}
DESC=${DESC:-bc_rnn_tx_chunk8_fullhist_c3000}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnTxC8FH_c3000v2_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
...
RESUME=""
if [ "$SMOKE" != "1" ]; then
  MARK=logs/bcrnnTxC8FH_c3000v2/.launched
  if [ -f "$MARK" ]; then
    LAST=$(ls -t /coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/bcrnnTxC8FH_c3000v2/*/checkpoints/last.ckpt 2>/dev/null | head -1)
    [ -n "$LAST" ] && RESUME="ckpt_path=$LAST"
  fi
  mkdir -p logs/bcrnnTxC8FH_c3000v2 && touch "$MARK"
fi

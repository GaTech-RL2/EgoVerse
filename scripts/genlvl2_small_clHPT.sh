...
#SBATCH --requeue
#SBATCH --output=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/genlvl40_smallCL/%x_%A_%a.out
#SBATCH --error=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/genlvl40_smallCL/%x_%A_%a.err
set -uo pipefail
cd /coc/flash7/paphiwetsa3/projects/EgoVerse2
source /coc/flash7/paphiwetsa3/projects/EgoVerse7/.venv/bin/activate
export PYTHONPATH=. MUJOCO_GL=egl HYDRA_FULL_ERROR=1
# SMALL-CIRCLE zero-shot variant of the genlvl 40-seed sweep: identical to genlvl2_40seed.sh
# EXCEPT every rollout adds ++evaluator.env_kwargs.pusher_shape=circle_small (the embodiment swap).
# Params via --export: CELL, RUN/RDIR, DATASET, FAMILY (hpt|packed), MODEL (packed only)
EXTRA=${EXTRA:-}
case "$CELL" in
...
RD=${RDIR:-$(ls -dt logs/${RUN}/*/ 2>/dev/null | head -1)}
...
VID=logs/genlvl40_smallCL_vids/${CELL}/lvl${LVL}; mkdir -p "$VID"; export ROLLOUT_VIDEO_DIR=$VID
echo "CELL=$CELL FAMILY=$FAMILY LVL=$LVL pusher=circle_small RD=$RD VID=$VID"

run_hpt() {
  srun python -m egomimic.trainHydra --config-name=train_zarr_cartesian \
    name=g2s_${CELL}_L${LVL} mode=eval data=tsimulation model=hpt_pushshapes_circle \
    ...
    "+evaluator.rollout_mode=chunk_openloop" "+evaluator.chunk_k=1" "+evaluator.temporal_ensemble=false" \
    "++evaluator.env_kwargs.obstacle_level=${LVL}" "++evaluator.env_kwargs.pusher_shape=circle_small" evaluator.max_steps=1800 "++evaluator.video_fps=30" "++evaluator.max_videos=1" \
...
run_packed() {
  srun python -m egomimic.trainHydra --config-name=train_zarr_cartesian \
    name=g2s_${CELL}_L${LVL} mode=eval data=tsimulation model=${MODEL} ++model.robomimic_model.action_horizon=1800 \
    ckpt_path=${CK} evaluator=eval_hnet_sim evaluator.rollout_mode=ar trainer.precision=32 \
    evaluator.init_mode=seeds "+evaluator.init_seeds=${SEEDS}" \
    "++evaluator.env_kwargs.obstacle_level=${LVL}" "++evaluator.env_kwargs.pusher_shape=circle_small" evaluator.max_steps=1800 "++evaluator.video_fps=30" "++evaluator.max_videos=1" \
    evaluator.coverage_threshold=0.95 evaluator.limit_val_batches=1 trainer.limit_val_batches=1 \
    trainer=debug trainer.profiler=null logger=csv \

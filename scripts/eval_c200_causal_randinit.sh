#!/bin/bash
#SBATCH --job-name=evalC200CausalRI
...
#SBATCH --output=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/sbatch/evalC200CausalRI_%j.out
#SBATCH --error=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/sbatch/evalC200CausalRI_%j.err
set -uxo pipefail
...
export ROLLOUT_VIDEO_DIR=${RUNDIR}/eval/video_chunkol_k32_randinit_thr95_ep899
mkdir -p "$ROLLOUT_VIDEO_DIR"

# GENERALIZATION eval: RANDOM-NEW init positions (init_mode=seeds, env-generated, NOT from the
# training dataset) + FIXED goal = dataset target [256,256,pi/4] (fixed_goal override, since the
# fixed-goal model only ever saw that one target). chunk_openloop k=32, threshold 0.95.
srun python -m egomimic.trainHydra \
  --config-name=train_zarr_cartesian \
  name=evalC200CausalRI description=eval_c200_causal_chunkol_k32_randinit_ep899 mode=eval \

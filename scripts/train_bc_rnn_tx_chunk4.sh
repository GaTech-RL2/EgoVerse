...
# peak 1e-4) -- the ONLY change is model=bc_rnn_pushshapes_paperexact_tx_chunk4,
# which adds OBS STRIDING (obs_stride=4) + ACTION CHUNKING (chunk_len=4) on top of
...
# windows over the SUBSAMPLED obs (obs_stride=4). With stride 4 each episode yields
# ~4x fewer obs-steps, so an epoch processes ~4x fewer obs-step windows than a
...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_tx_chunk4}
NAME=${NAME:-bcrnnTxC4_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnTxC4_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else

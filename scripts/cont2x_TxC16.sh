#!/bin/bash
#SBATCH --job-name=bcrnnTxC16FHx2
...
# ---- CONT-2x of bcrnnTxC16FH_nc3 (weights-only init + fresh productive warmup-cosine) ----
INIT_CKPT=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/bcrnnTxC16FH_nc3/bc_rnn_tx_chunk16_fullhist_relaunch_2026-06-10_02-55-13/checkpoints/epoch_epoch=299.ckpt
NORM=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/bcrnnTxC16FH_nc3/bc_rnn_tx_chunk16_fullhist_relaunch_2026-06-10_02-55-13/norm_stats/norm_stats.json
NAME=bcrnnTxC16FH_nc3_cont2x
DESC=bc_rnn_tx_chunk16_fullhist_cont2x
EPOCHS=${EPOCHS:-1800}; VALEVERY=25; LTB=50; MAXSTEPS=400
MAXSTEPS_SCHED=$(( EPOCHS * LTB ))   # fresh cosine horizon = new epochs x LTB
EXTRA_ARGS="model.robomimic_model.rnn_horizon=40 model.robomimic_model.core_net.max_window=40 +model.robomimic_model.window_anchor=start"   # per-run hydra overrides (rnn_horizon / window_anchor)

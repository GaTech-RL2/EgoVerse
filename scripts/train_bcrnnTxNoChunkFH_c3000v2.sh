...
# bcrnnTxNoChunkFH_c3000v2: FRESH full-history circle_3000 run. Cloned from the original
# new_circle_3 launcher recipe; ONLY the dataset folder_path + name changed.
# Model: bc_rnn_pushshapes_paperexact_tx_cos; chunk_len=1; FH override rnn_horizon=640.
...
SMOKE=${SMOKE:-0}
NAME=${NAME:-bcrnnTxNoChunkFH_c3000v2}
DESC=${DESC:-bc_rnn_tx_nochunk_fullhist_c3000}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnTxNoChunkFH_c3000v2_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else

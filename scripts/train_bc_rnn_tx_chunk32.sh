...
# peak 1e-4) -- the ONLY change is model=bc_rnn_pushshapes_paperexact_tx_chunk32,
# which adds OBS STRIDING (obs_stride=32) + ACTION CHUNKING (chunk_len=32) on top of
...
# ~32x fewer obs-steps, so an epoch processes ~32x fewer obs-step windows than a
...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_tx_chunk32}
NAME=${NAME:-bcrnnTxC32_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnTxC32_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else

...
KM=egomimic.rldb.embodiment.pushshapes.get_keymap_goal_eval
...
srun --kill-on-bad-exit=1 python -m egomimic.trainHydra \
  ...
  evaluator.goal_in_obs=true \
  evaluator.max_steps=null \
  evaluator.coverage_threshold=0.8 \
  data.train_datasets.pushshapes_sim.resolver.key_map._target_=egomimic.rldb.embodiment.pushshapes.get_keymap_goal \

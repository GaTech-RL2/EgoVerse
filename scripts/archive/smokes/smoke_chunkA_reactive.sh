...
python -m egomimic.trainHydra \
  ...
  name=hnet_baseline_nc3 description=smoke_chunkA_reactive mode=train data=tsimulation \
  model=hnet_pushshapes_fused_pusher_resnet \
  ++model.robomimic_model.reactive=true \
...
echo "SMOKE_CHUNKA_REACTIVE_EXIT=$?"

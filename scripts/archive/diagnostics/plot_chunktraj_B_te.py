#!/usr/bin/env python3
"""Plot chunk-B + temporal-ensemble EXECUTED cursor trajectories from chunktraj_te
npz dumps. One combined PNG: per episode a row of 2 panels.
  (a) 2D executed path x vs y, scatter+line colored by timestep, faint chunk-boundary
      markers at t=0,K,2K,... (would-be boundaries, for reference only -- TE has no
      hard boundary).
  (b) x(t), y(t) vs t, light vertical gridlines every K steps.
Also prints raw numeric diagnostics (within-window vs at-boundary step magnitudes).
"""
import os, glob
import numpy as np
import matplotlib
...
D = "/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/chunktraj_chunkB_te"
OUT = os.path.join(D, "chunkB_te_executed_path.png")

files = sorted(glob.glob(os.path.join(D, "chunktraj_te_ep*.npz")),
               key=lambda p: int(p.split("ep")[-1].split(".")[0]))
files = files[:4]
assert files, "no chunktraj_te npz found"
...
for r, (epi, a, K, cov) in enumerate(eps):
    ...
    # boundary step s (0-based diff a_s->a_{s+1}) is one LANDING on t=K,2K,... i.e.
    # (s+1) % K == 0. All other steps are "within-window".
    ...
    print(f"ep{epi}: T={T} K={K} cov={cov:.3f} | "
          f"mean|a_t+1-a_t| within-window={mi:.2f} boundary={mb:.2f} | "
    ...
    if bnd_steps.size:
        bvals = ", ".join(f"t{(s+1)}:{step_mag[s]:.1f}" for s in bnd_steps)
        print(f"      per-boundary |step|: {bvals}")
    ...
    axa.scatter(a[bnd_t, 0], a[bnd_t, 1], facecolors="none", edgecolors="red",
                s=70, linewidths=1.0, alpha=0.5, zorder=4,
                label="would-be chunk boundary (t=0,K,2K,...)")
    ...
    for bt in range(0, T + 1, K):
        axb.axvline(bt, color="gray", ls="--", lw=0.6, alpha=0.45)
    axb.set_xlabel("timestep t")
    axb.set_ylabel("cursor coord (world px)")
    axb.set_title(f"ep{epi}  x(t),y(t)  (dashed=would-be chunk bnd every {K})")
    axb.legend(fontsize=8, loc="best")

fig.suptitle("chunk-B + temporal-ensemble EXECUTED cursor trajectory  (rollout_mode=chunk_te, K=32)",
             fontsize=13, y=1.0)

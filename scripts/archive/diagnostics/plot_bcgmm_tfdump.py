...
D = sys.argv[1] if len(sys.argv) > 1 else "/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/tf_dump_bcgmm_ep100"
TAG = sys.argv[2] if len(sys.argv) > 2 else "epoch-100"
files = sorted(glob.glob(f"{D}/tf_ep*.npz"))
n = len(files)
if n == 0:
    print("NO NPZ in", D); sys.exit(1)
...
for i, f in enumerate(files):
    ...
    ax.plot(pred[:, 0], pred[:, 1], "-", color="red", lw=1.5, alpha=0.85, label="pred (GMM mode)")
    ...
    ax2.set_title(f"ep{i} per-frame L2 error"); ax2.set_xlabel("frame"); ax2.set_ylabel("L2 px err"); ax2.legend(fontsize=8)
fig.suptitle(f"BC-chunk + GMM (teacher-forced, obs-only) -- GT demo (green) vs GMM-mode pred (red)\nfused_pusher_resnet, chunk_k=32, action_head=gmm M=5; {TAG}", fontsize=13)
fig.tight_layout(rect=[0, 0, 1, 0.96])
out = f"{D}/bcgmm_tf_obsonly_gt_vs_pred.png"

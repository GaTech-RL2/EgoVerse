#!/bin/bash
#SBATCH --job-name=hptFlowComboEma
...
#   model=hpt_bc_flow_pushshapes_ema  (GroupNorm ResNet stem + RandomCrop86/CenterCrop86 augs)
#   callbacks=checkpoints_ema         (EMA shadow weights -> ema_state_dict in every ckpt)
#   name/description=hptFlowComboEma
...
NORM=${NORM:-/coc/flash7/paphiwetsa3/projects/EgoVerse2/external_ckpts/hptFlowC3000_circle3000_norm_stats.json}

SMOKE=${SMOKE:-0}
DESC=${DESC:-hptFlowComboEma}
NAME=${NAME:-hptFlowComboEma}
if [ "$SMOKE" = "1" ]; then
  NAME=hptFlowComboEma_smoke; EPOCHS=2; VALEVERY=2; LTB=2; LVB=2; LOGGER=csv

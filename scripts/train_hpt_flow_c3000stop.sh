#!/bin/bash
#SBATCH --job-name=hptFlowC3000CausalStop
...
NAME=hptFlowC3000CausalStop
# Same config as hptFlowC3000Causal (run 3334349) — ONLY the dataset changes to circle_3000_stop.
# Reuse the original run's norm_stats (loader-visible data ~identical to circle_3000).
...
DS=/coc/flash7/paphiwetsa3/datasets/circle_3000_stop
...
srun python -m egomimic.trainHydra \
  ...
  name=${NAME} description=hpt_flow_circle_3000_stop_causal_obs1_act32 mode=train \

#!/bin/bash
#SBATCH --job-name=hptFlowC3000Act8JepaV2
...
srun python -m egomimic.trainHydra \
  ...
  name=${NAME} description=hpt_flow_circle_3000_act8_jepa_v2_vicreg_lambda1 mode=train \
  data=tsimulation_hpt_causal_jepa model=hpt_pushshapes_circle_jepa \
  ++model.robomimic_model.jepa.lambda=1.0 \

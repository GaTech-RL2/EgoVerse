#!/bin/bash
#SBATCH --job-name=pact-pixel-dfot
...
#SBATCH --qos=short
#SBATCH --no-requeue
#SBATCH --output=/coc/flash7/paphiwetsa3/projects/EgoVerse-pact/logs/sbatch/pact_pixel_%j.out
#SBATCH --error=/coc/flash7/paphiwetsa3/projects/EgoVerse-pact/logs/sbatch/pact_pixel_%j.err
...
srun --kill-on-bad-exit=1 .venv/bin/python -m egomimic.trainHydra \
  ...
  name=pact_pixel_dfot description=pixel_dfot_9frame_postfix \
  mode=train \
  ...
  model=dfot_pushshapes_pixel \
  model.scheduler.max_steps=100000 \
  ...
  trainer.limit_train_batches=80 data.train_dataloader_params.pushshapes_sim.batch_size=2 data.valid_dataloader_params.pushshapes_sim.batch_size=2 \

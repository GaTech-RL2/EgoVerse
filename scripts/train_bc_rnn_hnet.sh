...
# BC-RNN-HNET. IDENTICAL recipe to train_bc_rnn_tx_cos.sh (same budget
# EPOCHS/LTB, fp32 trainer.precision=32, no grad clip, repeat-pad unmasked
...
# single-knob with the TX-cos run 3324263.
#
...
# ----- Batch composition + budget (identical to tx_cos / paperexact) -----
...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_hnet}
NAME=${NAME:-bcrnnHnet_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnHnet_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
  ...
  # checkpoints across the run (+ 'last'). (H-Net per-step wall time may differ
  # from the LSTM/TX; the budget is kept identical so the builds get the same
  # optimizer-step / window-gradient budget -- adjust EPOCHS only if the measured
  # A40 step time changes materially.)

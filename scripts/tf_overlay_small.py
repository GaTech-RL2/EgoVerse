...
OUTDIR = os.environ.get("OUTDIR", f"{RUN}/videos/small_tf_overlay")
...
def overlay_for_episode(ep_path, out_mp4):
    ...
    mode = os.environ.get("PRED_MODE", "step")  # 'step' (windowed, trained regime) | 'full'
    with torch.no_grad():
        if mode == "full":
            # full-episode recurrent unroll (forward_eval) -- single zero-init
            # hidden for the WHOLE episode. OOD for a model trained windowed at
            # rnn_horizon=10, so it degenerates; kept for comparison only.
            unnorm = algo.forward_eval(proc)
            pred_n = algo.norm_stats.normalize(
                {"actions": torch.zeros((n, 2))}, emb_id)  # placeholder unused
            pred_w = unnorm[f"emb{emb_id}_{ac_key}"][0].detach().cpu().numpy()
        else:
            # WINDOWED teacher-forced: drive policy.step frame-by-frame exactly
            # like training/inference (re-inits the hidden every rnn_horizon obs
            # steps). This is the dense per-step prediction in the regime the
            # model was actually trained on -- the honest TF overlay.
            st = policy.init_step_state(batch_size=1, T_max=n, device=dev)
            sa = proc[emb_id]["state_agent_obj"]   # (n,5) normalized
            im = proc[emb_id]["front_img_1"]       # (n,3,96,96) normalized
            preds_n = []
            for t in range(n):
                # match _format_pushshapes_obs: (B,D) state, (B,C,H,W) image; the
                # obs_encoder auto-unsqueezes the T=1 dim.
                obs_t = {"state_agent_obj": sa[t:t+1],     # (1,5)
                         "front_img_1": im[t:t+1]}         # (1,3,96,96)
                a = policy.step(st, obs_t, t)              # (1,1,2) normalized
                preds_n.append(a.reshape(-1))
            pred_norm = torch.stack(preds_n, 0)    # (n,2)
            pred_w = algo.norm_stats.unnormalize(
                {ac_key: pred_norm}, emb_id)[ac_key].detach().cpu().numpy()
    if os.environ.get("DIAG"):
        clean0 = ~((gt_actions[:,0]<=2)|(gt_actions[:,0]>=510)|(gt_actions[:,1]<=2)|(gt_actions[:,1]>=510))
        gx_, gy_ = gt_actions[clean0,0], gt_actions[clean0,1]
        px_, py_ = pred_w[clean0,0], pred_w[clean0,1]
        def corr(a,b): return float(np.corrcoef(a,b)[0,1]) if len(a)>2 else float("nan")
        print(f"  [DIAG] pred range x[{pred_w[:,0].min():.0f},{pred_w[:,0].max():.0f}] "
              f"y[{pred_w[:,1].min():.0f},{pred_w[:,1].max():.0f}]  "
              f"gt range x[{gt_actions[:,0].min():.0f},{gt_actions[:,0].max():.0f}] "
              f"y[{gt_actions[:,1].min():.0f},{gt_actions[:,1].max():.0f}]")
        print(f"  [DIAG] corr(px,gx)={corr(px_,gx_):.2f} corr(py,gy)={corr(py_,gy_):.2f} "
              f"corr(px,gy)={corr(px_,gy_):.2f} corr(py,gx)={corr(py_,gx_):.2f}")
        for t in (0, 30, 60, 100, 150):
            if t < n:
                print(f"  [DIAG] t={t:3d}  gt=({gt_actions[t,0]:.0f},{gt_actions[t,1]:.0f})  "
                      f"pred=({pred_w[t,0]:.0f},{pred_w[t,1]:.0f})  "
                      f"agent=({state[t,0]:.0f},{state[t,1]:.0f})")
    # ---- metrics ----
    # GT actions are the raw CURSOR position, which has ~13% off-screen GARBAGE
    # frames (mouse leaves the canvas -> action snaps to (0,0) or arena edge).
    # The dense MSE is DOMINATED by those GT-garbage frames, not model error, so
    # report BOTH the raw RMSE and a ROBUST RMSE that masks garbage GT frames:
    #   - GT exactly (0,0) (off-screen sentinel), or
    #   - GT within 2px of an arena edge (cursor pinned to border), or
    #   - GT->GT jump > 80px between consecutive frames (teleport = off-canvas).
    per_err = np.linalg.norm(pred_w - gt_actions, axis=1)  # (T,) per-frame L2 px
    g = gt_actions
    edge = (g[:, 0] <= 2) | (g[:, 0] >= 510) | (g[:, 1] <= 2) | (g[:, 1] >= 510)
    zero = (np.abs(g).sum(1) < 1.0)
    jump = np.zeros(n, bool)
    d = np.linalg.norm(np.diff(g, axis=0), axis=1)
    jump[1:] = d > 80
    garbage = edge | zero | jump
    clean = ~garbage
    mse = float(np.mean((pred_w - gt_actions) ** 2))
    rmse = float(np.sqrt(mse))
    rmse_clean = float(np.sqrt(np.mean(per_err[clean] ** 2))) if clean.any() else float("nan")
    med_clean = float(np.median(per_err[clean])) if clean.any() else float("nan")
    frac_g = float(garbage.mean())
    print(f"  {os.path.basename(ep_path)}  T={n}  raw_RMSE={rmse:.1f}  "
          f"CLEAN_RMSE={rmse_clean:.2f}px  CLEAN_median={med_clean:.2f}px  garbage_frac={frac_g:.2f}")
    # ---- render overlay ----
    ...
    for t in range(n):
        ...
        px, py = pred_w[t] * s * SC
        # only trail/draw GT when it's a CLEAN (on-canvas) frame; pred always.
        if not garbage[t]:
            gt_trail.append((int(gx), int(gy)))
        pr_trail.append((int(px), int(py)))
        ...
        tag = "GARBAGE-GT" if garbage[t] else f"err={per_err[t]:.0f}px"
        cv2.putText(frame,
                    f"GT(grn) PRED(red) t={t}/{n} cleanRMSE={rmse_clean:.0f} {tag}",
                    (8, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1, cv2.LINE_AA)
        vw.write(frame)
    vw.release()
    return mse, rmse, rmse_clean, med_clean, frac_g

VIDNAME = os.environ.get("VIDNAME", "overlay_small_ep")
results = []
for i, idx in enumerate(EP_IDXS[:N_EP]):
    ep = ep_files[idx]
    out = os.path.join(OUTDIR, f"{VIDNAME}{i}.mp4")
    mse, rmse, rmse_clean, med_clean, frac_g = overlay_for_episode(ep, out)
    results.append((os.path.basename(ep), rmse, rmse_clean, med_clean, frac_g, out))

print("\n=== TF OVERLAY SUMMARY (CLEAN = GT-garbage masked) ===")
for b, rmse, rmse_clean, med_clean, frac_g, out in results:
    print(f"  {b}  raw_RMSE={rmse:.1f}  CLEAN_RMSE={rmse_clean:.2f}px  "
          f"CLEAN_median={med_clean:.2f}px  garbage={frac_g:.2f}  -> {out}")
cr = np.array([r[2] for r in results]); md = np.array([r[3] for r in results])
print(f"[agg] mean CLEAN_RMSE={cr.mean():.2f}px  mean CLEAN_median={md.mean():.2f}px")
print("[ref] action std ~110px/dim -> mean-baseline RMSE ~155px; tight tracking << that")

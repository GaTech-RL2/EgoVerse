#!/bin/bash
#SBATCH --job-name=HnetC8FHRcontR
...
# ---- GENTLE-TAIL CONTINUATION of bcrnnHnetC8FHR_nc3 ----
...
ORIG=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/bcrnnHnetC8FHR_nc3/bc_rnn_hnet_chunk8_fullhist_ratio_2026-06-06_22-31-32
INIT_CKPT=$ORIG/checkpoints/epoch_epoch=1699.ckpt
NORM=$ORIG/norm_stats/norm_stats.json

NAME=bcrnnHnetC8FHR_nc3_cont
DESC=bc_rnn_hnet_chunk8_fullhist_ratio_cont
EPOCHS=1800; VALEVERY=100; LTB=50; MAXSTEPS=400

# ---- REQUEUE-SAFE checkpoint selection ----
# Recovery ckpt: the newest last.ckpt found at relaunch time (epoch 1699).
RECOVERY_CKPT=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/bcrnnHnetC8FHR_nc3_cont/bc_rnn_hnet_chunk8_fullhist_ratio_cont_2026-06-08_23-12-35/checkpoints/last.ckpt
# If this run has written its own newer last.ckpt (e.g. after a requeue), pick it up.
LAST=$(ls -t /coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/${NAME}/*/checkpoints/last.ckpt 2>/dev/null | head -1)
if [ -n "$LAST" ]; then CKPT_ARG="ckpt_path=$LAST"; else CKPT_ARG="ckpt_path=$RECOVERY_CKPT"; fi
echo "RESUMING FROM: $CKPT_ARG"

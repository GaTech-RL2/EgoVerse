...
# BC-RNN-HNET-CHUNK8-QUERIES (QUERY decoder, obs_stride=8/chunk_len=8).
...
# The ONLY change is model=bc_rnn_pushshapes_paperexact_hnet_chunk8_q (chunk_head: queries (vs LINEAR bcrnnHnetC8)). Core stays the REAL
...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_hnet_chunk8_queries}
NAME=${NAME:-bcrnnHnetC8Q_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnHnetC8Q_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
...
srun --kill-on-bad-exit=1 python -m egomimic.trainHydra \
  ...
  model=bc_rnn_pushshapes_paperexact_hnet_chunk8_q \

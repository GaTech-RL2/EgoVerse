#!/bin/bash
#SBATCH --job-name=bcrnnHnetC16FH_c3000v2
...
# bcrnnHnetC16FH_c3000v2: FRESH full-history circle_3000 run. Cloned from the original
# new_circle_3 launcher recipe; ONLY the dataset folder_path + name changed.
# Model: bc_rnn_pushshapes_paperexact_hnet_chunk16; chunk_len=16; FH override rnn_horizon=40.
...
SMOKE=${SMOKE:-0}
NAME=${NAME:-bcrnnHnetC16FH_c3000v2}
DESC=${DESC:-bc_rnn_hnet_chunk16_fullhist_c3000}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnHnetC16FH_c3000v2_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
...
RESUME=""
if [ "$SMOKE" != "1" ]; then
  MARK=logs/bcrnnHnetC16FH_c3000v2/.launched
  if [ -f "$MARK" ]; then
    LAST=$(ls -t /coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/bcrnnHnetC16FH_c3000v2/*/checkpoints/last.ckpt 2>/dev/null | head -1)
    [ -n "$LAST" ] && RESUME="ckpt_path=$LAST"
  fi
  mkdir -p logs/bcrnnHnetC16FH_c3000v2 && touch "$MARK"
fi

srun --kill-on-bad-exit=1 python -m egomimic.trainHydra \
  --config-name=train_zarr_cartesian \
  name=${NAME} description=${DESC} mode=train data=tsimulation \
  model=bc_rnn_pushshapes_paperexact_hnet_chunk16 \
  ...
  model.robomimic_model.rnn_horizon=40 model.robomimic_model.core_net.max_window=40 model.robomimic_model.window_anchor=start ${RESUME}
echo "TRAIN_EXIT=$?"

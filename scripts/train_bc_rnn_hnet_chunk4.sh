...
# BC-RNN-HNET-CHUNK4 (LINEAR, obs_stride=4/chunk_len=4).
...
# The ONLY change is model=bc_rnn_pushshapes_paperexact_hnet_chunk4 (obs_stride/chunk_len 8 -> 4, LINEAR chunk head). Core stays the REAL
...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_hnet_chunk4}
NAME=${NAME:-bcrnnHnetC4_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnHnetC4_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else

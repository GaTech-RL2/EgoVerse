#!/bin/bash
#SBATCH --job-name=hnet-fused-80ep-cosine
...
#SBATCH --output=/coc/flash7/paphiwetsa3/projects/EgoVerse/logs/sbatch/hnet_fused_80ep_cosine_%j.out
#SBATCH --error=/coc/flash7/paphiwetsa3/projects/EgoVerse/logs/sbatch/hnet_fused_80ep_cosine_%j.err

# H-Net Fused (hnet_pushshapes_fused) — 50 epochs, warmup+cosine LR schedule:
...
srun --kill-on-bad-exit=1 .venv/bin/python -m egomimic.trainHydra \
  ...
  name=hnet_variants \
  description=hnet_fused_80ep_cosine_lr4e5 \
  mode=train \
  data=tsimulation \
  model=hnet_pushshapes_fused \

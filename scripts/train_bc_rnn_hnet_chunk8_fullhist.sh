...
# BC-RNN-HNET-CHUNK8-FULLHIST (variant A, bcrnnHnetC8FH). SAME recipe as
# bcrnnHnetC8 (train_bc_rnn_hnet_chunk8.sh) with FULL-EPISODE HISTORY: the model
# config bc_rnn_pushshapes_paperexact_hnet_chunk8_fullhist sets rnn_horizon=80
...
# stats, warmup->cosine LR peak 1e-4 -- all identical to bcrnnHnetC8.
#
...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_hnet_chunk8_fullhist}
NAME=${NAME:-bcrnnHnetC8FH_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnHnetC8FH_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
  # ----- Budget (same as bcrnnHnetC8; A40 fp32) -----

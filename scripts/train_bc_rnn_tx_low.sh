...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_tx_cos_lowlr}
NAME=${NAME:-bcrnnTxLow_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnTxLow_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else

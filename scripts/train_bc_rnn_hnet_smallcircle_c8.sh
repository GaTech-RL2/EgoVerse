#!/bin/bash
#SBATCH --job-name=bcrnnHnetSmallC8
...
# SMALL-CIRCLE window-10 H-Net chunk-8 WITH closed-loop COVERAGE eval (shows on wandb).
# Derived from the big-circle bcrnnHnetC8 launcher (evaluator=eval_hnet_sim, default
# valid setup), retargeted to new_circle_small__3 + the small pusher
# (++evaluator.env_kwargs.pusher_shape=circle_small, radius auto 5.0). Reuses the small
# minmax norm stats. RESUME=<ckpt> to full-resume.
...
DESC=${DESC:-bc_rnn_hnet_smallcircle_c8}
NAME=${NAME:-bcrnnHnetSmallC8_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnHnetSmallC8_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else

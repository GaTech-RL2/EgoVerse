#!/bin/bash
#SBATCH --job-name=evalC3000CausalRI
...
#SBATCH --output=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/sbatch/evalC3000CausalRI_%j.out
#SBATCH --error=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/sbatch/evalC3000CausalRI_%j.err
set -uxo pipefail
...
# training dataset). c3000 is goal-conditioned, so the env samples a RANDOM goal from the
# arena distribution (in-distribution for this model) and the model reaches for it.
# chunk_openloop k=32, threshold 0.95.
srun python -m egomimic.trainHydra \
  --config-name=train_zarr_cartesian \
  name=evalC3000CausalRI description=eval_c3000_causal_chunkol_k32_randinit_ep1799 mode=eval \

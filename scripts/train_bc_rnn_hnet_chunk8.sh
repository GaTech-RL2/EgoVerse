...
# BC-RNN-HNET-CHUNK8 (variant B, bcrnnHnetC8). The exact H-Net TWIN of bcrnnTxC8:
# a SINGLE-KNOB clone of train_bc_rnn_tx_chunk8.sh (same budget EPOCHS/LTB, fp32
# trainer.precision=32, no grad clip, repeat-pad unmasked windows, raw low-dim obs
# + ReLU image + no fusion MLP, no actor MLP, eval RANDOM crop with v0.2 off-by-one,
# minmax norm fresh full-data stats, warmup->cosine LR peak 1e-4) -- the ONLY change
# is model=bc_rnn_pushshapes_paperexact_hnet_chunk8, which swaps the recurrent core
# TransformerCore -> the user's REAL dynamic-chunking H-Net while KEEPING obs_stride=8
# + chunk_len=8 (linear chunk head, rnn_horizon=10). So bcrnnTxC8 vs this run
# isolates the core (TX vs H-Net) under identical obs-striding + action-chunking.
#
...
# (Matches the tx_chunk8 / paperexact_hnet template this is cloned from; the
# train_bc_rnn_{cos,base}.sh variants that DO pass precomputed_norm_path use the
# OLD circle dataset's precomputed stats, which do not apply to new_circle_3.)
...
# stride-1 run -- EPOCHS/LTB are kept IDENTICAL to bcrnnTxC8 for comparability.
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_hnet_chunk8}
NAME=${NAME:-bcrnnHnetC8_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnHnetC8_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else

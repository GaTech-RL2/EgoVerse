#!/bin/bash
#SBATCH --job-name=hnet-adaln-qreg-50ep
...
#SBATCH --output=/coc/flash7/paphiwetsa3/projects/EgoVerse/logs/sbatch/hnet_adaln_qreg_50ep_%j.out
#SBATCH --error=/coc/flash7/paphiwetsa3/projects/EgoVerse/logs/sbatch/hnet_adaln_qreg_50ep_%j.err
...
srun --kill-on-bad-exit=1 .venv/bin/python -m egomimic.trainHydra \
  ...
  name=hnet_variants \
  description=hnet_adaln_50ep_cosine_lr4e5_qreg \
  mode=train \

#!/bin/bash
#SBATCH --job-name=bcrnnHnetC8FH_cotrain
...
# bcrnnHnetC8FH_cotrain: FRESH full-history circle_3000 run. Cloned from the original
# new_circle_3 launcher recipe; ONLY the dataset folder_path + name changed.
# Model: bc_rnn_pushshapes_paperexact_hnet_chunk8_fullhist; chunk_len=8; FH override rnn_horizon=baked.
...
SMOKE=${SMOKE:-0}
NAME=${NAME:-bcrnnHnetC8FH_cotrain}
DESC=${DESC:-bc_rnn_hnet_chunk8_fullhist_cotrain}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnHnetC8FH_cotrain_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
...
RESUME=""
if [ "$SMOKE" != "1" ]; then
  MARK=logs/bcrnnHnetC8FH_cotrain/.launched
  if [ -f "$MARK" ]; then
    LAST=$(ls -t /coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/bcrnnHnetC8FH_cotrain/*/checkpoints/last.ckpt 2>/dev/null | head -1)
    [ -n "$LAST" ] && RESUME="ckpt_path=$LAST"
  fi
  mkdir -p logs/bcrnnHnetC8FH_cotrain && touch "$MARK"
fi

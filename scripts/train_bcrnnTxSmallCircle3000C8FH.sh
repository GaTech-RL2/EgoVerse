#!/bin/bash
#SBATCH --job-name=bcrnnTxSmallCircle3000C8FH
...
# BC small_circle_3000 baseline for TransformerAR. Faithful clone of train_bcrnnTxC8FH_c3000v2.sh:
...
# Model: bc_rnn_pushshapes_paperexact_tx_chunk8_fullhist; fresh minmax on small_circle_3000.

SMOKE=${SMOKE:-0}
NAME=${NAME:-bcrnnTxSmallCircle3000C8FH}
DESC=${DESC:-bc_rnn_tx_chunk8_fullhist_smallcircle3000}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnTxSmallCircle3000C8FH_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; PROJ=""; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
...
RESUME=""
if [ "$SMOKE" != "1" ]; then
  MARK=logs/bcrnnTxSmallCircle3000C8FH/.launched
  if [ -f "$MARK" ]; then
    LAST=$(ls -t /coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/bcrnnTxSmallCircle3000C8FH/*/checkpoints/last.ckpt 2>/dev/null | head -1)
    [ -n "$LAST" ] && RESUME="ckpt_path=$LAST"
  fi
  mkdir -p logs/bcrnnTxSmallCircle3000C8FH && touch "$MARK"
fi

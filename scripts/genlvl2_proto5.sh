...
#SBATCH --requeue
#SBATCH --output=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/genlvl5/%x_%A_%a.out
#SBATCH --error=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/genlvl5/%x_%A_%a.err
set -uo pipefail
...
SEEDS='[0,1,2,3,4]'
VID=logs/genlvl5_vids/${CELL}/lvl${LVL}; mkdir -p "$VID"; export ROLLOUT_VIDEO_DIR=$VID
...
run_hpt() {
  srun python -m egomimic.trainHydra --config-name=train_zarr_cartesian \
    ...
    evaluator.coverage_threshold=1.01 evaluator.limit_val_batches=1 trainer.limit_val_batches=1 \
...
run_packed() {
  srun python -m egomimic.trainHydra --config-name=train_zarr_cartesian \
    name=g2_${CELL}_L${LVL} mode=eval data=tsimulation model=${MODEL} ++model.robomimic_model.action_horizon=1800 \

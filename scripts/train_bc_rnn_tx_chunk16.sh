...
# peak 1e-4) -- the ONLY change is model=bc_rnn_pushshapes_paperexact_tx_chunk16,
...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_tx_chunk16}
NAME=${NAME:-bcrnnTxC16_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnTxC16_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else

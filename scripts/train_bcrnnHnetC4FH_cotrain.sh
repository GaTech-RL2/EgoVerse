#!/bin/bash
#SBATCH --job-name=bcrnnHnetC4FH_cotrain
...
DATA=/coc/flash7/paphiwetsa3/datasets/circle_3000_plus_cotrain240
KM=egomimic.rldb.embodiment.pushshapes.get_keymap_eval

# bcrnnHnetC4FH_cotrain: FRESH full-history circle_3000 run. Cloned from the original
# new_circle_3 launcher recipe; ONLY the dataset folder_path + name changed.
# Model: bc_rnn_pushshapes_paperexact_hnet_chunk4; chunk_len=4; FH override rnn_horizon=160.
...
SMOKE=${SMOKE:-0}
NAME=${NAME:-bcrnnHnetC4FH_cotrain}
DESC=${DESC:-bc_rnn_hnet_chunk4_fullhist_cotrain}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnHnetC4FH_cotrain_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
  EPOCHS=${EPOCHS:-1800}; VALEVERY=${VALEVERY:-100}; LTB=${LTB:-50}; LOGGER=wandb; MAXSTEPS=900; SEEDS=""; export WANDB_MODE=online
fi
...
RESUME=""
if [ "$SMOKE" != "1" ]; then
  MARK=logs/bcrnnHnetC4FH_cotrain/.launched
  if [ -f "$MARK" ]; then
    LAST=$(ls -t /coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/bcrnnHnetC4FH_cotrain/*/checkpoints/last.ckpt 2>/dev/null | head -1)
    [ -n "$LAST" ] && RESUME="ckpt_path=$LAST"
  fi
  mkdir -p logs/bcrnnHnetC4FH_cotrain && touch "$MARK"
fi

srun --kill-on-bad-exit=1 python -m egomimic.trainHydra \
  --config-name=train_zarr_cartesian \
  name=${NAME} description=${DESC} mode=train data=tsimulation \
  model=bc_rnn_pushshapes_paperexact_hnet_chunk4 \

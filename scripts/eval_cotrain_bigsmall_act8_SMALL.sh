#!/bin/bash
#SBATCH --job-name=evalCotrainBigSmallAct8_SMALL
...
#SBATCH --output=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/sbatch/evalCotrainBigSmallAct8_SMALL_%j.out
#SBATCH --error=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/sbatch/evalCotrainBigSmallAct8_SMALL_%j.err
set -uxo pipefail
...
export ROLLOUT_VIDEO_DIR=${RUNDIR}/eval/video_SMALLcircle_chunkol_k8_randinit_thr95
mkdir -p "$ROLLOUT_VIDEO_DIR"

# SMALL circle (pusher_shape=circle_small, r=5 auto-resolved from PUSHER_RADII). Goal-conditioned ->
# env samples a random in-distribution goal per seed. random-init generalization, chunk_openloop k=8, thr 0.95, 20 seeds.
srun python -m egomimic.trainHydra \
  --config-name=train_zarr_cartesian \
  name=evalCotrainBigSmallAct8_SMALL description=eval_cotrain_bigsmall_act8_SMALLcircle_chunkol_k8_randinit mode=eval \

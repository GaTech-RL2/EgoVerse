...
#SBATCH --output=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/sbatch/fusedar_%x_%j.out
#SBATCH --error=/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/sbatch/fusedar_%x_%j.err
set -euxo pipefail
...
# AR = scheduled-sampling: feeds back the model's OWN predicted action at rate
# ss_prob, so it learns to recover from its own errors. token_dropout=0 so the
# fed-back action is actually used (1.0 would drop it). chunk_k=1 (single-step AR).
...
srun --kill-on-bad-exit=1 python -m egomimic.trainHydra \
  ...
  +model.robomimic_model.ss_prob=${SSPROB} \

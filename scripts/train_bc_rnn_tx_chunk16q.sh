...
# BC-RNN-TRANSFORMER-CHUNK8-QUERIES. SINGLE-KNOB clone of train_bc_rnn_tx_chunk16.sh
...
# warmup->cosine LR peak 1e-4, obs_stride=16/chunk_len=16) -- the ONLY change is
# model=bc_rnn_pushshapes_paperexact_tx_chunk16_q, which swaps the chunk readout
# from the flat wide-linear gmm_head projection to an ACT/HPT-style ACTION-QUERY
# DECODER (chunk_head: queries). So bcrnnTxC16 vs this run isolates the
...
# windows over the SUBSAMPLED obs (obs_stride=16). With stride 16 each episode yields
...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_tx_chunk16_queries}
NAME=${NAME:-bcrnnTxC16Q_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnTxC16Q_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
...
srun --kill-on-bad-exit=1 python -m egomimic.trainHydra \
  ...
  model=bc_rnn_pushshapes_paperexact_tx_chunk16_q \

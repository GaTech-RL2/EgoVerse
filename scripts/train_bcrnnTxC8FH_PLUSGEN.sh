#!/bin/bash
#SBATCH --job-name=bcrnnTxC8FHPlusGen
...
# +gen arm: BC-RNN-TX chunk-8 full-history, SAME recipe as the converged -gen run
# (bcrnnTxC8FH_c3000v2), only the dataset differs -> circle_3000_plus_cotrain240.
DATA=/coc/flash7/paphiwetsa3/datasets/circle_3000_plus_cotrain240
KM=egomimic.rldb.embodiment.pushshapes.get_keymap_eval

SMOKE=${SMOKE:-0}
NAME=${NAME:-bcrnnTxC8FHPlusGen}
DESC=${DESC:-bc_rnn_tx_chunk8_fullhist_PLUSGEN}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnnTxC8FHPlusGen_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; PROJ=""; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
...
RESUME=""
if [ "$SMOKE" != "1" ]; then
  MARK=logs/bcrnnTxC8FHPlusGen/.launched
  if [ -f "$MARK" ]; then
    LAST=$(ls -t /coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/bcrnnTxC8FHPlusGen/*/checkpoints/last.ckpt 2>/dev/null | head -1)
    [ -n "$LAST" ] && RESUME="ckpt_path=$LAST"
  fi
  mkdir -p logs/bcrnnTxC8FHPlusGen && touch "$MARK"
fi

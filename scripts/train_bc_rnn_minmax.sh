...
SMOKE=${SMOKE:-0}
DESC=${DESC:-bc_rnn_minmax}
NAME=${NAME:-bcrnn_minmax_nc3}
if [ "$SMOKE" = "1" ]; then
  NAME=bcrnn_minmax_smoke; EPOCHS=2; VALEVERY=2; LTB=6; LOGGER=csv; MAXSTEPS=20; SEEDS="evaluator.init_seeds=[0,1]"; export WANDB_MODE=disabled
else
...
srun --kill-on-bad-exit=1 python -m egomimic.trainHydra \
  ...
  model=bc_rnn_pushshapes_minmax \

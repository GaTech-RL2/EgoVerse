#!/bin/bash
#SBATCH --job-name=hpt-50ep-cosine
...
#SBATCH --output=/coc/flash7/paphiwetsa3/projects/EgoVerse/logs/sbatch/hpt_50ep_cosine_%j.out
#SBATCH --error=/coc/flash7/paphiwetsa3/projects/EgoVerse/logs/sbatch/hpt_50ep_cosine_%j.err
...
srun --kill-on-bad-exit=1 .venv/bin/python -m egomimic.trainHydra \
  ...
  name=hpt_variants \
  description=hpt_50ep_cosine_lr4e5 \
  mode=train \

...
Loads the ep1599 checkpoint, runs the FHR H-Net core teacher-forced over a few
...
def main():
    ...
    fig.suptitle("FHR ep1599 boundary_prob per obs-step  (target N=%.1f, frac=%.2f)" %
                 (target_N, 1.0/target_N), fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    p1 = out / "FHR_boundary_diag_ep1599_strip.png"
    ...
    ax2.set_xlabel("obs-step index"); ax2.set_title("FHR ep1599 boundary_prob heatmap (bright=boundary)")
    fig2.colorbar(im, ax=ax2, fraction=0.025, pad=0.01, label="P(boundary)")
    fig2.tight_layout()
    p2 = out / "FHR_boundary_diag_ep1599_heatmap.png"
    ...
    ax3.set_title("FHR ep1599 boundary_prob distribution\nfrac>0.5=%.3f (target %.3f)" %
                  (overall_frac, 1.0/target_N))
    ax3.legend(); fig3.tight_layout()
    p3 = out / "FHR_boundary_diag_ep1599_hist.png"

...
class LocalEpisodeResolverWithEmbodimentOverride(LocalEpisodeResolver):
    ...
    The override is applied post-resolve on each returned ZarrDataset.
    Because ``ZarrDataset.__getitem__`` derives the per-sample
    ``data["embodiment"]`` from ``self.embodiment`` lazily, overriding the
    attribute before any sample is loaded is enough.
    """

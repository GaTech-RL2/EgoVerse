...
HUMAN_ACTION_SLOWDOWN = (ACTION_HORIZON - 1) / (
    HUMAN_ACTION_SOURCE_HORIZON - 1
)
...
class HeadFramePose(Transform):
    ...
    def transform(self, batch):
        ...
        position = np.einsum(
            "tij,tj->ti", inverse_head, pose[:, :3] - head[:, :3]
        )
        rotation = np.einsum(
...
def eva_normal_keymap(norm_mode=False, annotation_key=None):
    keymap = {
        "front_img_1": {"key_type": "camera_keys", "zarr_key": "images.front_1", "horizon": N_OBS_STEPS},
        "front_intrinsics": {"key_type": "metadata_keys", "zarr_key": "intrinsics.front_1"},
        "left_camera_extrinsics": {"key_type": "metadata_keys", "zarr_key": "extrinsics.left"},
        "right_camera_extrinsics": {"key_type": "metadata_keys", "zarr_key": "extrinsics.right"},
        "left.cmd_ee_pose": {"key_type": "action_keys", "zarr_key": "left.cmd_ee_pose", "horizon": _ROBOT_ACTION_FETCH},
        "right.cmd_ee_pose": {"key_type": "action_keys", "zarr_key": "right.cmd_ee_pose", "horizon": _ROBOT_ACTION_FETCH},
        "left.cmd_gripper": {"key_type": "action_keys", "zarr_key": "left.cmd_gripper", "horizon": _ROBOT_ACTION_FETCH},
        "right.cmd_gripper": {"key_type": "action_keys", "zarr_key": "right.cmd_gripper", "horizon": _ROBOT_ACTION_FETCH},
        "left.obs_ee_pose": {"key_type": "proprio_keys", "zarr_key": "left.obs_ee_pose", "horizon": N_OBS_STEPS},
        "right.obs_ee_pose": {"key_type": "proprio_keys", "zarr_key": "right.obs_ee_pose", "horizon": N_OBS_STEPS},
        "left.obs_gripper": {"key_type": "proprio_keys", "zarr_key": "left.obs_gripper", "horizon": N_OBS_STEPS},
        "right.obs_gripper": {"key_type": "proprio_keys", "zarr_key": "right.obs_gripper", "horizon": N_OBS_STEPS},
        "left_wrist_img": {"key_type": "camera_keys", "zarr_key": "images.left_wrist", "horizon": N_OBS_STEPS},
        "right_wrist_img": {"key_type": "camera_keys", "zarr_key": "images.right_wrist", "horizon": N_OBS_STEPS},
    }
...
def human_normal_keymap(norm_mode=False, annotation_key=None):
    keymap = {
        "front_img_1": {"key_type": "camera_keys", "zarr_key": "images.front_1", "horizon": N_OBS_STEPS},
        "front_intrinsics": {"key_type": "metadata_keys", "zarr_key": "intrinsics.front_1"},
        "left.act_ee_pose": {"key_type": "action_keys", "zarr_key": "left.obs_ee_pose", "horizon": _HUMAN_ACTION_FETCH},
        "right.act_ee_pose": {"key_type": "action_keys", "zarr_key": "right.obs_ee_pose", "horizon": _HUMAN_ACTION_FETCH},
        "left.obs_ee_pose": {"key_type": "proprio_keys", "zarr_key": "left.obs_ee_pose", "horizon": N_OBS_STEPS},
        "right.obs_ee_pose": {"key_type": "proprio_keys", "zarr_key": "right.obs_ee_pose", "horizon": N_OBS_STEPS},
        "obs_head_pose": {"key_type": "proprio_keys", "zarr_key": "obs_head_pose", "horizon": N_OBS_STEPS},
    }
    return _drop_camera_keys(keymap) if norm_mode else keymap
...
def human_normal_keymap_kp(norm_mode=False, annotation_key=None):
    keymap = {
        "front_img_1": {"key_type": "camera_keys", "zarr_key": "images.front_1", "horizon": N_OBS_STEPS},
        "front_intrinsics": {"key_type": "metadata_keys", "zarr_key": "intrinsics.front_1"},
        "left.act_keypoints": {"key_type": "proprio_keys", "zarr_key": "left.obs_keypoints", "horizon": _HUMAN_ACTION_FETCH},
        "right.act_keypoints": {"key_type": "proprio_keys", "zarr_key": "right.obs_keypoints", "horizon": _HUMAN_ACTION_FETCH},
        "left.obs_keypoints": {"key_type": "proprio_keys", "zarr_key": "left.obs_keypoints", "horizon": N_OBS_STEPS},
        "right.obs_keypoints": {"key_type": "proprio_keys", "zarr_key": "right.obs_keypoints", "horizon": N_OBS_STEPS},
        "left.obs_wrist_pose": {"key_type": "proprio_keys", "zarr_key": "left.obs_wrist_pose", "horizon": N_OBS_STEPS},
        "right.obs_wrist_pose": {"key_type": "proprio_keys", "zarr_key": "right.obs_wrist_pose", "horizon": N_OBS_STEPS},
        "obs_head_pose": {"key_type": "proprio_keys", "zarr_key": "obs_head_pose", "horizon": N_OBS_STEPS},
    }
    return _drop_camera_keys(keymap) if norm_mode else keymap
...
def eva_normal_transforms():
    return [
        ...
        ActionChunkCoordinateFrameTransform("left.current_ee_pose", "left.cmd_ee_pose", "left.cmd_ee_wrist", mode="xyzwxyz"),
        ActionChunkCoordinateFrameTransform("right.current_ee_pose", "right.cmd_ee_pose", "right.cmd_ee_wrist", mode="xyzwxyz"),
        PoseToRot6D("left.cmd_ee_wrist"),
        PoseToRot6D("right.cmd_ee_wrist"),
        PoseToRot6D("left.obs_ee_pose"),
        PoseToRot6D("right.obs_ee_pose"),
        ConcatKeys(["left.cmd_ee_wrist", "left.cmd_gripper", "right.cmd_ee_wrist", "right.cmd_gripper"], "actions_cartesian", delete_old_keys=True),
        ConcatKeys(["left.obs_ee_pose", "left.obs_gripper", "right.obs_ee_pose", "right.obs_gripper"], "state_ee_pose", delete_old_keys=True),
        DropKeys(["left.cmd_ee_pose", "right.cmd_ee_pose", "left.current_ee_pose", "right.current_ee_pose"]),
...
def human_normal_transforms():
    transforms = [
        ...
        ActionChunkCoordinateFrameTransform("left.current_ee_pose", "left.act_ee_pose", "left.action_wrist", mode="xyzwxyz"),
        ActionChunkCoordinateFrameTransform("right.current_ee_pose", "right.act_ee_pose", "right.action_wrist", mode="xyzwxyz"),
        InterpolatePose(ACTION_HORIZON, "left.action_wrist", "left.action_wrist", stride=1, mode="xyzwxyz"),
        InterpolatePose(ACTION_HORIZON, "right.action_wrist", "right.action_wrist", stride=1, mode="xyzwxyz"),
        ...
        ConcatKeys(["left.action_wrist", "left.action_gripper", "right.action_wrist", "right.action_gripper"], "actions_cartesian", delete_old_keys=True),
        ZerosLike("left.obs_head", "left.obs_gripper"),
        ZerosLike("right.obs_head", "right.obs_gripper"),
        ConcatKeys(["left.obs_head", "left.obs_gripper", "right.obs_head", "right.obs_gripper"], "state_ee_pose", delete_old_keys=True),
        DropKeys(["left.act_ee_pose", "right.act_ee_pose", "left.current_ee_pose", "right.current_ee_pose", "left.obs_ee_pose", "right.obs_ee_pose"]),
...
def human_normal_transforms_kp():
    return [
        ...
        ActionChunkCoordinateFrameTransform("left.current_wrist_pose", "left.act_keypoints", "left.action_keypoints", mode="xyz"),
        ActionChunkCoordinateFrameTransform("right.current_wrist_pose", "right.act_keypoints", "right.action_keypoints", mode="xyz"),
        InterpolatePose(ACTION_HORIZON, "left.action_keypoints", "left.action_keypoints", stride=1, mode="xyz"),
        InterpolatePose(ACTION_HORIZON, "right.action_keypoints", "right.action_keypoints", stride=1, mode="xyz"),
        ReshapePoints("left.action_keypoints", flatten=True),
        ReshapePoints("right.action_keypoints", flatten=True),
        ConcatKeys(["left.action_keypoints", "right.action_keypoints"], "actions_keypoints", delete_old_keys=True),
        ...
        ConcatKeys(["left.current_wrist_head", "right.current_wrist_head"], "viz_current_wrist_poses", delete_old_keys=True),
        HeadFrameKeypoints("obs_head_pose", "left.obs_keypoints", "left.obs_keypoints_head"),
        HeadFrameKeypoints("obs_head_pose", "right.obs_keypoints", "right.obs_keypoints_head"),
        ConcatKeys(["left.obs_keypoints_head", "right.obs_keypoints_head"], "state_keypoints", delete_old_keys=True),
        DropKeys(["left.current_wrist_pose", "right.current_wrist_pose", "left.act_keypoints", "right.act_keypoints", "left.obs_keypoints", "right.obs_keypoints", "left.obs_wrist_pose", "right.obs_wrist_pose"]),
        NumpyToTensor(keys=["actions_keypoints", "state_keypoints", "obs_head_pose", "viz_current_wrist_poses"]),
    ]
...
class Rot6DToPoseYPR(Transform):
    ...
    def transform(self, batch):
        ...
        batch[self.out_key] = np.concatenate([value[..., :3], ypr], axis=-1).astype(np.float32)
        return batch


def build_bimanual_rot6d_wrist_revert_transforms(action_key="actions_cartesian", state_key="state_ee_pose"):
    return [
        SplitConcat(action_key, [("left.action_pose", 9), ("left.action_grip", 1), ("right.action_pose", 9), ("right.action_grip", 1)]),
        SplitConcat(state_key, [("left.state_pose", 9), ("left.state_grip", 1), ("right.state_pose", 9), ("right.state_grip", 1)], delete_old_key=False),
...
class BimanualKeypointWristRevert(Transform):
    ...
    def transform(self, batch):
        ...
        if actions.shape[-1] == 126:
            ...
            pieces = [self._apply(left, wrists[:7]).reshape(-1, 63), self._apply(right, wrists[7:]).reshape(-1, 63)]
            ...
            pieces = [self._apply(left_wrist, wrists[:7]).reshape(-1, 3), self._apply(left, wrists[:7]).reshape(-1, 63), self._apply(right_wrist, wrists[7:]).reshape(-1, 3), self._apply(right, wrists[7:]).reshape(-1, 63)]
        else:
            raise ValueError("keypoint actions must use canonical 126-D or legacy 132-D")
        batch[self.action_key] = np.concatenate(pieces, axis=-1).astype(np.float32)
        return batch


def build_bimanual_keypoint_wrist_revert_transforms(action_key="actions_keypoints", wrist_pose_key="viz_current_wrist_poses"):
...
def eva_action_revert_transforms(in_key="actions_cartesian", out_key="robot_action"):
    return [
        SplitConcat(in_key, [("left.pose", 9), ("left.grip", 1), ("right.pose", 9), ("right.grip", 1)]),
        Rot6DToPoseYPR("left.pose"),
        Rot6DToPoseYPR("right.pose"),
        ConcatKeys(["left.pose", "left.grip", "right.pose", "right.grip"], out_key, delete_old_keys=True),
    ]

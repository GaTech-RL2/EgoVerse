...
class Embodiment(ABC):
    ...
    def get_keymap(
        ...
        Role-named annotation arrays (``annotations_task`` /
        ``annotations_subtask``) need no registration here — ZarrDataset
        fetches every ``annotations*`` key automatically; ``annotation_key``
        remains only as a legacy alias registration.
        """

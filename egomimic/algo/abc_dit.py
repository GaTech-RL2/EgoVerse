"""ABC-DiT: the algo wrapping ``ABCDiTPolicy`` (Amazon FAR's ABC, ported).

Standalone by design: ABC-DiT conditions differently from FlowVLA (vision via
cross-attention, ``[state, task, time]`` via a 9-way AdaLN) and carries ABC's
own training knobs -- state masking and action-prefix conditioning -- so it
owns its batch handling rather than borrowing another algo's chassis. What it
shares with the rest of the repo is the batch contract: HPT-style batches
(native action layout, dotted dataset keys, proprio ``(B, D)`` or ``(B, K, D)``
from the ``proprio_history`` keymap) in, and the unnormalised-prediction
contract ``eval_hpt.py`` reads out.

Design notes and the size ladder: ``egomimic/models/abc_dit_nets.py``.
"""
...
class ABCDiT(DeterministicEvalMixin, Algo):
    @property
    def device(self):
        return self._device
    ...
    def __init__(
        self,
        norm_stats,
        policy,  # functools.partial of ABCDiTPolicy (hydra ``_partial_: true``)
        domains: list,
        dims: dict,
        ac_keys: dict,
        ...
        history_len: int = 1,
        proprio_keys: Optional[list] = None,
        **kwargs,
    ):
        ...
        # Which of the data config's proprio keys make up the state vector.
        # ``None`` = every one the keymap declares; ABC-DiT concatenates them
        # into ONE state vector, so an unwanted key (data/mecka's 20-D
        # ``observations.state.ee_pose`` next to the 144-D keypoints) is named
        # out here rather than dropped by having no stem for it.
        self.selected_proprio_keys = (
            None if proprio_keys is None else list(proprio_keys)
        )
        ...
        # Read by eval_hpt.py; ABC-DiT has one head, no auxiliary / shared keys.
        self.shared_ac_key = None
        self.auxiliary_ac_keys = {}
        # Construct on the CPU: Lightning assigns the rank's GPU only after the
        # model exists and writes it to ``device``, whose setter moves the nets
        # (``ModelWrapper.on_fit_start`` / ``on_validation_start``). Building on
        # "cuda" would put every DDP rank's weights on cuda:0 until then.
        ...
        if not state_dims:
            raise ValueError(
                "ABC-DiT conditions on proprio; every domain's dims need a "
                f"'proprio' width (got {self.dims})"
            )
        self.nets["policy"] = policy(
        ...
        for embodiment in self.domains:
            ...
            for key in norm_stats.keys_of_type("proprio_keys", embodiment_id):
                if not norm_stats.is_key_with_embodiment(key, embodiment_id):
                    continue
                if (
                    self.selected_proprio_keys is not None
                    and key not in self.selected_proprio_keys
                ):
                    continue
                self.proprio_keys[embodiment_id].append(key)
            if self.selected_proprio_keys is not None:
                missing = [
                    k
                    for k in self.selected_proprio_keys
                    if k not in self.proprio_keys[embodiment_id]
                ]
                if missing:
                    raise KeyError(
                        f"proprio_keys {missing} are not proprio keys of the norm "
                        f"stats for {embodiment}; it has "
                        f"{sorted(norm_stats.keys_of_type('proprio_keys', embodiment_id))}"
                    )
            ...
            if embodiment_id not in self.ac_keys:
                raise KeyError(
                    f"ac_keys[{embodiment}]={self.ac_keys[embodiment]!r} is not an "
                    "action key of the norm stats"
                )
            cameras = self.camera_keys[embodiment_id]
            roles = list(self.nets["policy"].camera_roles)
            if cameras != roles:
                raise ValueError(
                    f"{embodiment}'s cameras {cameras} do not match the policy's "
                    f"camera_roles {roles}; the policy holds one attention-pool "
                    "query set and camera embedding per role, in this order"
                )
    ...
    def compose_prompt(self, embodiment_name: str, annotation: str) -> str:
        """The CLIP prompt. ABC feeds the bare task text, so ``embodiment_label``
        defaults off here: the text tower is frozen, which makes the prompt a
        read-only channel and a poor way to send a categorical variable."""
        blocks = []
        ...
        if annotation:
            blocks.append(f"Task: {annotation}" if blocks else annotation)
        return " ".join(blocks)
    ...
    def _to_model_data(self, _batch: dict, embodiment_id: int) -> dict:
        """Processed batch -> the dict ``ABCDiTPolicy`` consumes."""
        ...
        if not frames:
            raise KeyError(
                f"no camera of {self.camera_keys[embodiment_id]} in the batch "
                f"keys {sorted(_batch)}"
            )
        ...
        if not parts:
            raise KeyError(
                f"ABC-DiT needs proprio; none of {self.proprio_keys[embodiment_id]} "
                f"is in the batch keys {sorted(_batch)}"
            )
        steps = {p.shape[1] for p in parts}
        if len(steps) != 1:
            raise ValueError(f"proprio keys carry different history lengths: {steps}")
    ...
    def forward_eval(self, batch):
        ...
        with torch.no_grad():
            for embodiment_id, _batch in batch.items():
                ...
                ac_key = self.ac_keys[embodiment_id]
                data = self._to_model_data(_batch, embodiment_id)
                # The val loss draws noise / t from the global RNG inside the
                # policy: fork + seed it so the reported loss is reproducible.

"""BC-RNN policy/algo for EgoVerse -- faithful robomimic BC-RNN(-GMM) recipe.
...
* ROLLOUT HIDDEN RE-INIT EVERY `rnn_horizon` STEPS. robomimic
  ...
  indefinitely). We replicate this exactly in ``BCRNNPolicy.step`` via a step
        ...
        if self.chunk_head == "queries":
            if query_decoder is None:
                raise ValueError(
                    ...
                    "(instantiated by BCRNN under the model 'query_decoder:' "
...
class BCRNN(HNet):
    def __init__(
        ...
        **kwargs,
        ...
        if int(getattr(gmm_head, "chunk_len", 1)) != self.chunk_len:
            raise ValueError(
                f"BCRNN.chunk_len ({self.chunk_len}) must equal "
                f"gmm_head.chunk_len ({getattr(gmm_head, 'chunk_len', 1)}); set "
        ...
        # window_anchor: "uniform" (default, pre-existing -- all valid (b, s)
        # start frames, subsampled to max_windows_per_batch) or "start"
        # (full-history: exactly ONE window per episode anchored at frame 0, so
        # the training context matches a NEVER-RESET rollout). See _cut_windows.
        if window_anchor not in ("uniform", "start"):
            raise ValueError(
                f"window_anchor must be uniform|start, got {window_anchor!r}"
            )
        self.window_anchor = str(window_anchor)
        # collect_ratio_loss: DEFAULT False == EXACTLY today's behavior (the
        # forward_training emits ratio_loss = 0.0, a placeholder, so the
        # inherited HNet.compute_losses' ``total = action_loss + ratio_loss``
        # adds nothing). True == the H-Net PAPER recipe: read the HNetCore's
        # collected chunker ratio loss (``core._last_ratio_loss``, already
        # weighted by the core's ratio_loss_weight) and emit it as ratio_loss so
        # compute_losses folds the weighted aux into the optimized action_loss.
        # Only meaningful with core='hnet' (other cores have no chunker); guarded
        # at use-site in forward_training. Plumbed exactly like obs_stride /
        # window_anchor (a parsed BCRNN attribute threaded from the config).
        # NOTE: the core='hnet' guard for this knob lives AFTER ``self.core`` is
        # assigned below (this assignment has no self.core dependency).
        self.collect_ratio_loss = bool(collect_ratio_loss)
        ...
        if core not in ("lstm", "transformer", "hnet"):
            raise ValueError(f"core must be lstm|transformer|hnet, got {core!r}")
        self.core = str(core)
        # collect_ratio_loss is only meaningful with the H-Net core (only it has
        # a ChunkerStage that produces a ratio loss); fail loud on a mis-config.
        if self.collect_ratio_loss and self.core != "hnet":
            raise ValueError(
                "collect_ratio_loss=True requires core='hnet' (only the H-Net "
                f"core has a chunker / ratio loss); core is {self.core!r}."
            )
    ...
    def forward_training(self, batch):
        ...
        for emb_id, _batch in batch.items():
            ...
            # H-Net PAPER ratio loss (config-gated). DEFAULT (collect_ratio_loss
            # False): emit the 0.0 PLACEHOLDER -- byte-identical to the prior
            # behavior, so the inherited HNet.compute_losses' ``total = aloss +
            # rloss`` is unchanged for every non-ratio config. ON: read the
            # value the HNetCore stashed during ``policy(obs_w)`` above
            # (``core._last_ratio_loss``), which ``ratio_loss_from_aux`` already
            # WEIGHTED by the core's ratio_loss_weight (so do NOT re-weight here
            # -- matches the hnet.py algo convention that compute_losses adds the
            # pre-weighted r straight onto the action loss). The aux graph is
            # live, so this term backprops into the chunker's boundary router.
            if self.collect_ratio_loss:
                rloss = policy.lstm._last_ratio_loss
                if rloss is None:
                    raise RuntimeError(
                        "collect_ratio_loss=True but the HNetCore did not stash "
                        "a ratio loss this forward (core._last_ratio_loss is "
                        "None); expected core='hnet' with a ChunkerStage."
                    )

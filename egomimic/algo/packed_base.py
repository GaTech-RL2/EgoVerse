...
class HNetFused(HNet):
    ...
    Isotropic stack, no chunker / ratio loss) lived in
    ``FlatFusedOuterStage``, which was QUARANTINED to
    ``scratch/flat_fused_quarantine/`` (B-family legacy, unreferenced by
    pact-2's hourglass mission; see scratch/MANIFEST.md). This class is now
    DORMANT dead code -- no config can construct it. Kept only so a resurrected
    flat-fused config still resolves its ``_target_``.
    """

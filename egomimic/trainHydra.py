...
def _metric_frames_per_episode(cfg: DictConfig, head: str) -> int | None:
    """``data.metric_frames_per_episode[head]`` (K), or None when this config
    does not ask for subsampling on this head."""
...
def _subsample_val_datasets(cfg: DictConfig, head: str, datasets: dict) -> dict:
    ...
    made the metric window the first ``limit_val_batches * batch_size`` frames
    in episode-hash order: a third of the seen-val split and effectively one
    operator of the unseen split (lane-0 coverage report, 2026-09-15). K evenly
    spaced frames per episode gives every episode -- and so every operator --
    equal weight at the same batch count.

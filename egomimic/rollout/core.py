...
class RolloutNode:
    ...
    def __repr__(self) -> str:
        return (f"{type(self).__name__}(reads={list(self.reads)}, "
...
class RolloutPipeline:
    ...
    def __init__(self, nodes: List[RolloutNode],
                 seed_keys: Sequence[str] = ("t", "obs")):
    ...
    def validate(self) -> None:
        ...
        if problems:
            lines = [f"  {name}: missing {miss} (available: {avail})"
    ...
    def reset(self, state: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        state.update({"t": 0, "queue": [], "chunk": None, "chunk_t": None,
                      "prev_chunk": None, "action": None, "command": None})
        for n in self.nodes:
            n.reset(state)
        return state
    ...
    def explain(self) -> str:
        ...
        for n in self.nodes:
            ...
            out.append("%-22s reads=%-42s writes=%s%s" % (

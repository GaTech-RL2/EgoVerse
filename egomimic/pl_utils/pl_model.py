...
class ModelWrapper(LightningModule):
    ...
    def on_train_epoch_start(self):
        ...
        # Check parameter gradient status (especially for epoch 0 and 1 to catch issues)
        # if self.current_epoch <= 1:
        #     memutils.print_parameter_gradient_status(self.model, epoch=self.current_epoch, print_all=False)
        
        # # Finally, log memory usage in MB
        # mem_stats = memutils.get_memory_summary()
        # print("\nEpoch {} Memory Usage: {} GB\n".format(self.current_epoch, mem_stats['rss_gb']))
        # # self.log('epoch', self.trainer.current_epoch)
        # self.log("System/RAM Usage (GB)", mem_stats['rss_gb'], sync_dist=True)
        
        # # Store memory stats for final summary
        # epoch_stat = {
        #     "epoch": self.current_epoch,
        #     **mem_stats  # Includes rss_gb, cuda_alloc_gb, cuda_reserved_gb, cuda_peak_gb
        # }
        # self.epoch_memory_stats.append(epoch_stat)

        return super().on_train_epoch_start()

    # @rank_zero_only
    # def on_train_end(self):
    #     """Print final memory statistics summary at the end of training."""
    #     memutils.print_epoch_memory_summary(self.epoch_memory_stats)

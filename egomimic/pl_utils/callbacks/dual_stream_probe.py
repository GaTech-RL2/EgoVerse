...
class DualStreamProbeCallback(Callback):
    ...
    def _classify(cls, name: str):
        ...
        if "action_out" in name:
            return "head", cls._stream(name)   # A_head/S_head (proj_A/proj_S); gate -> None
    ...
    def on_after_backward(self, trainer, pl_module):
        ...
        for key, v in sq.items():
            pl_module.log(
                f"Train/grad_norm_{key}", v ** 0.5,
                on_step=False, on_epoch=True, sync_dist=False,  # per-rank diagnostic; no DDP allreduce
            )
        ...
        for m in pl_module.modules():
            ...
            if wbe:  # single shared head dispatched over all embodiments
                for emb, (wa, ws) in wbe.items():
                    pl_module.log(f"Train/wA_{emb}", float(wa),
                                  on_step=False, on_epoch=True, sync_dist=False)
                    pl_module.log(f"Train/wS_{emb}", float(ws),
                                  on_step=False, on_epoch=True, sync_dist=False)
                m._w_by_emb = {}
                m._fired = False
                ...
                pl_module.log(
                    f"Train/wA_{emb}", float(getattr(m, "_last_w_A", 0.0)),
                    on_step=False, on_epoch=True, sync_dist=False,  # dynamic per-emb key must NOT allreduce
                )
                pl_module.log(
                    f"Train/wS_{emb}", float(getattr(m, "_last_w_S", 0.0)),
                    on_step=False, on_epoch=True, sync_dist=False,
                )
                m._fired = False

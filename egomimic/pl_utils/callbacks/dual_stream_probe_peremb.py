...
class DualStreamPerEmbProbeCallback(Callback):
    ...
    def on_after_backward(self, trainer, pl_module):
        for key, v in self._bucket_grads(pl_module).items():
            pl_module.log(
                f"Train/grad_norm_{key}", v,
                on_step=False, on_epoch=True, sync_dist=False,
            )
        for m in pl_module.modules():
            if getattr(m, "_fired", False):
                emb = getattr(m, "_emb_id", "x")
                pl_module.log(f"Train/wA_{emb}", float(getattr(m, "_last_w_A", 0.0)),
                              on_step=False, on_epoch=True, sync_dist=False)
                pl_module.log(f"Train/wS_{emb}", float(getattr(m, "_last_w_S", 0.0)),
                              on_step=False, on_epoch=True, sync_dist=False)
                m._fired = False
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        try:
            ...
            if getattr(trainer, "is_global_zero", True):
                print(f"[PEREMB_PROBE] skipped step={pl_module.global_step}: {type(e).__name__}: {e}", flush=True)
        finally:
    ...
    def _per_emb_probe(self, pl_module, batch):
        ...
        emb_losses = {k[: -len("_action_loss")]: v

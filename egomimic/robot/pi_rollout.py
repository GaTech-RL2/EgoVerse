...
Array2D = Any  # np.ndarray or torch.Tensor of shape (T, D)
...
class PolicyRollout(Rollout):
    def __init__(self, arm, policy_path, query_frequency, cartesian, extrinsics_key, resampled_action_len=None):
        ...
        self.policy = ModelWrapper.load_from_checkpoint(policy_path, weights_only=False, map_location="cpu")
        ...
        self.extrinsics = CameraTransforms(intrinsics_key="base", extrinsics_key=extrinsics_key).extrinsics
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.policy.model.nets.to(self.device)
        self.debug_actions = None
    ...
    def reset(self):
        self.actions = None
        self.debug_actions = None
        self.policy = ModelWrapper.load_from_checkpoint(self.policy_path, weights_only=False, map_location="cpu")
        self.policy = self.policy.to('cuda')
        if getattr(self.policy.model, "diffusion", False):
            for head in self.policy.model.nets.policy.heads:
                if isinstance(self.policy.model.nets.policy.heads[head], DenoisingPolicy):
...
if __name__ == "__main__":
    ...
    # breakpoint()
    # ModelWrapper.load_from_checkpoint(args.policy_path)
    # exit()

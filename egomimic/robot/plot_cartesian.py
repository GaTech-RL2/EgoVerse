...
def _infer_per_arm_dims(total_dims, num_arms):
  ...
  if d not in (6, 7, 8):  # 6: pos+euler, 7: +grip, 8: pos+quat(+grip?) assumed w/o grip
...
def plot_cartesian_3d(
  ...
  triad_len=0.05,
  max_triad_count=60,
  out_path="cartesian_plot.png", dpi=150, fmt="png"
):
  ...
  all_per_arm, d_per_arm = _split_actions_per_arm(
    A,
    arms_order if len(arms_order) > 1 else (['left', 'right'] if D % 2 == 0 else [arms])
  )
  ...
  for arm, arr in all_per_arm.items():
    ...
    for i in idxs:
      p = pos[i]
      R_i = Rmats[i]
      ax.quiver(p[0], p[1], p[2], *(R_i[:, 0] * triad_len), arrow_length_ratio=0.2)
      ax.quiver(p[0], p[1], p[2], *(R_i[:, 1] * triad_len), arrow_length_ratio=0.2)
      ax.quiver(p[0], p[1], p[2], *(R_i[:, 2] * triad_len), arrow_length_ratio=0.2)
...
def main():
  ap = argparse.ArgumentParser(description="Plot 6-DoF Cartesian actions (trajectory + local axes).")
  ap.add_argument("--h5", required=True, help="Path to dataset (HDF5).")
  ap.add_argument("--key", default="action", help="Dataset key to read (default: action).")
  ap.add_argument("--arms", default="right", choices=["left", "right", "both"])
  ap.add_argument("--layout", default="left-right", choices=["left-right", "right-left"],
                  help="Order of arms within each action vector if both arms are present.")
  ap.add_argument("--orient", default="auto", choices=["auto", "euler", "quat"],
                  help="Interpretation of orientation. 'auto': 6/7->euler, 8->quat.")
  ap.add_argument("--euler-seq", default="xyz",
                  help="Euler sequence if using Euler (default xyz; intrinsic).")
  ap.add_argument("--quat-w-first", action="store_true",
                  help="Set if quaternion stored as [w,x,y,z] (default assumes [x,y,z,w]).")
  ap.add_argument("--stride", type=int, default=None, help="Sample stride for drawing triads.")
  ap.add_argument("--triad-len", type=float, default=0.05, help="Axis length for triads.")
  ap.add_argument("--max-triads", type=int, default=60, help="Auto stride target triad count.")
  ...
  ap.add_argument("--format", default="png", choices=["png","pdf","svg","jpg","jpeg"])
  args = ap.parse_args()

...
class Robot_Interface(ABC):
    ...
    def __init__(self):
        ...
        self.ik = EvaKinematicsSolver(self.robot_urdf)
...
class SingleARXInterface(Robot_Interface):
    ...
    def set_joint(self, desired_position):
        ...
        requested.gripper_pos = float(
            desired_position[6] * self.gripper_width
        )  # TODO Elmo had a "- 0.018" here. Depends on calibration
    ...
    def get_obs(self):
        ...
        if self.arm == "right":
            obs["right_wrist_img"] = self.cameras["cam_right_wrist"].get_image()
        elif self.arm == "left":
            obs["left_wrist_img"] = self.cameras["cam_left_wrist"].get_image()
    ...
    def solve_ik(self, ee_pose):
        ...
        arm_joints = self.ik.inverse_kinematics(pos_xyz, quat_xyzw, self.get_joint())
        return arm_joints

...
import numpy as np
from aria_utils import AriaVRSExtractor, timestamp_ms_to_episode_hash
...
def main(args) -> None:
    ...
    try:
        zarr_paths = []
        mp4_paths = []
        for raw_path in Path(args.raw_path).glob("*.vrs"):
            episode_hash = timestamp_ms_to_episode_hash(int(raw_path.stem))

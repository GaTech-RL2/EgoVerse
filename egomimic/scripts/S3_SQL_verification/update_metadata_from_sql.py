...
def get_metadata_path_from_episode_hash(
    episode_hash: str, bucket_name: str = "rldb"
) -> Optional[str]:
    """
    ...
    for embodiment in df["embodiment"].unique():
        ...
        for _, row in df[df["embodiment"] == embodiment].iterrows():
            ...
            if timestamp_ms is None:
                print(
                    f"  Warning: Could not convert episode_hash {episode_hash} to timestamp"
                )
                continue
            ...
            if timestamp_ms in metadata_files:
                ...
                if not found:
                    print(
                        f"  Warning: Could not find metadata file for episode_hash {episode_hash} (timestamp_ms: {timestamp_ms})"
                    )

    return metadata_paths


def download_metadata_from_s3(s3_path: str, bucket_name: str, local_path: Path) -> bool:
...
def upload_metadata_to_s3(local_path: Path, s3_path: str, bucket_name: str) -> bool:
...
def update_metadata_json(metadata_path: Path, changes: Dict) -> Dict:
    """
            ...
            if table_key == "num_frames" and isinstance(value, str):
                if value == "" or value.lower() == "none":
                    ...
                    try:
                        ...
                        print(
                            f"  Warning: Could not convert num_frames '{value}' to int, skipping"
                        )
                        continue
...
def main():
    ...
    try:
        ...
        if args.sql_only:
            if not args.sql_changes:
                ...
                return 1
            sql_changes = parse_metadata_changes(
                args.sql_changes
            )  # Reuse parser, same format
    ...
    for idx, (episode_hash, s3_metadata_path) in enumerate(metadata_paths.items(), 1):
        ...
        if args.sql_only:
            ...
            if not download_metadata_from_s3(
                s3_metadata_path, args.bucket, local_metadata_path
            ):
                print("  Error: Failed to download metadata")
                error_count += 1
                continue

            # Update metadata
            try:
                updated_metadata = update_metadata_json(
                    local_metadata_path, metadata_changes
                )
            ...
            # Upload back to S3
            if not upload_metadata_to_s3(
                local_metadata_path, s3_metadata_path, args.bucket
            ):
                print("  Error: Failed to upload metadata")
                error_count += 1
                continue

            print("  Uploaded updated metadata to S3")

            # Update SQL table
            try:
                update_sql_table(engine, episode_hash, updated_metadata)
                print("  Updated SQL table entry")
                success_count += 1
            except Exception as e:
                print(f"  Error updating SQL table: {e}")
                error_count += 1

...
class AriaRay(EmbodimentRay):
    def __init__(
        ...
        self.raw_remote_prefix = os.environ.get(
            "RAW_REMOTE_PREFIX", "s3://rldb/raw_v2/test_aria"
        ).rstrip("/")
        ...
        self.resources_big = {"eva_big": 1}  # TODO: change to aria_big
        self.num_cpus_small = 2
        self.num_cpus_big = 8
        self.log_root = log_root
...
class EvaRay(EmbodimentRay):
    def __init__(
        ...
        self.raw_remote_prefix = os.environ.get(
            "RAW_REMOTE_PREFIX", "s3://rldb/raw_v2/test_eva"
        ).rstrip("/")
        self.processed_remote_prefix = os.environ.get(
            "PROCESSED_REMOTE_PREFIX", "s3://rldb/processed_v3/test_eva"
        ).rstrip("/")
        self.bucket = os.environ.get("BUCKET", "rldb")

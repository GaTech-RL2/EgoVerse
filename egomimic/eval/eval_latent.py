...
class PILatentEvalVideo(EvalVideo):
    ...
    def compute_metrics_and_viz(self, batch):
        ...
        with torch.no_grad():
            for embodiment_id, _batch in batch.items():
                ...
                unnorm_actions = algo.data_schematic.unnormalize_data(
                    predictions, embodiment_id
                )
                for k in unnorm_actions:
                    unnorm_preds[f"{embodiment_name}_{k}"] = unnorm_actions[k]

                unnorm_batch = algo.data_schematic.unnormalize_data(
                    _batch, embodiment_id
                )
    ...
    def find_existing_csv_dir(self, cfg) -> str | None:
        """In eval mode, look for a prior run with matching (model_type,
        model_time, description) under `latent_eval/<type>/<time>/`. Return
        the most recent epoch dir that contains per-layer CSVs, or None.

        Skipped if `force_reeval=true` is set on the CLI / config. Caller
        is responsible for routing to `rebuild_from_csvs` if a path is
        returned.
        """
        import os
        import re

        from egomimic.eval.latent_helpers import (
        ...
        parent = os.path.join("logs", str(name), "latent_eval", str(mtype), str(mtime))

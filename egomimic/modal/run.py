...
class _Config:
    ...
    @property
    def sync_script(self) -> str:
        return (
            f"{self.remote_repo_dir}/egomimic/scripts/data_download/sync_from_config.py"
        )
...
# ---------------------------------------------------------------------------
# CPU-only data sync function
# ---------------------------------------------------------------------------


_SYNC_CPU = float(os.environ.get("SYNC_CPU", os.environ.get("MODAL_CPU", "8.0")))
_SYNC_MEMORY_MB = (
    int(float(os.environ["SYNC_MEMORY_GB"]) * 1024)
    if os.environ.get("SYNC_MEMORY_GB")
    else (
        int(float(os.environ["MODAL_MEMORY_GB"]) * 1024)
        if os.environ.get("MODAL_MEMORY_GB")
        else 16384
    )
)
...
def run_sync_only(
    ...
    """Clone the repo at *git_commit* and run sync_from_config.py against the
    mounted egoverse-data volume. CPU-only — no GPU is allocated.

    Pre-populates the volume so a subsequent training run skips the S3 download.
    """
    _prepare_repo(git_remote=git_remote, git_commit=git_commit)

    cmd = [CFG.venv_python, CFG.sync_script, *hydra_args]
    ...
    if process.returncode != 0:
        raise RuntimeError(
            f"Sync failed (exit {process.returncode}): {shlex.join(cmd)}"
        )
...
@app.local_entrypoint()
def sync_data(*hydra_args: str) -> None:
    """Blocking CPU-only data sync against the egoverse-data Modal volume.

    Example:
        modal run egomimic/modal/run.py::sync_data -- \\
            --config-name=train_zarr_cartesian_pi data=mecka_pi
    """
    git_remote, git_commit, is_dirty = _resolve_git_state()
    if is_dirty:
        print(
    ...
    print(f"Syncing using commit {git_commit[:12]} from {git_remote}")
    run_sync_only.remote(tuple(hydra_args), git_remote, git_commit)
    print("Sync complete. egoverse-data volume is now populated.")

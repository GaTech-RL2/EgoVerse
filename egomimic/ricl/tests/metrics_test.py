...
Run under pytest or standalone:  python egomimic/ricl/tests/metrics_test.py
"""
...
def test_shuffle_ricl_keys_is_derangement():
    try:
        import torch
    except Exception:
        return  # torch optional
    B, k = 4, 3
    state = torch.arange(B).view(B, 1, 1).repeat(1, k, 2).float()  # row i is all i
    batch = {
        "eva_bimanual": {
            "actions_cartesian": torch.arange(B),  # non-ricl key: untouched
            "ricl_retrieved_state": state.clone(),
            "ricl_retrieved_mask": torch.ones(B, k, dtype=torch.bool),
        }
    }
    out = M.shuffle_ricl_keys(batch, seed=0)
    sub = out["eva_bimanual"]
    # non-ricl keys are passed through unchanged
    assert torch.equal(
        sub["actions_cartesian"], batch["eva_bimanual"]["actions_cartesian"]
    )
    # every query's neighbor block moved (derangement -> no row keeps its own value)
    rows = sub["ricl_retrieved_state"][:, 0, 0]
    assert not torch.any(rows == torch.arange(B).float())
    # the multiset of blocks is preserved (a pure permutation)
    assert sorted(rows.tolist()) == list(range(B))
    # original batch not mutated
    assert torch.equal(
        batch["eva_bimanual"]["ricl_retrieved_state"][:, 0, 0], torch.arange(B).float()
    )


def test_shuffle_ricl_keys_small_batch_returns_none():
    try:
        import torch
    except Exception:
        return  # torch optional
    batch = {"eva_bimanual": {"ricl_retrieved_state": torch.zeros(1, 3, 2)}}
    assert M.shuffle_ricl_keys(batch, seed=0) is None

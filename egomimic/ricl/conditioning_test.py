...
so it runs on a CPU-only box. Run under pytest, or standalone:
    ...
    python egomimic/ricl/conditioning_test.py
"""
...
def test_build_block_format_and_count():
    ...
    block = cond.build_retrieved_prompt_block(states, actions, action_steps=1)
    assert block.count("retrieved ") == k
    assert block.count(" | ") == k - 1
    assert "state " in block and "; action " in block
...
def test_build_block_valid_mask():
    ...
    block = cond.build_retrieved_prompt_block(states, actions, valid)
    assert block.count("retrieved ") == 2
    # all invalid -> empty
...
def test_build_block_action_steps_controls_length():
    ...
    n1 = len(b1.split("action ")[1].split())
    n3 = len(b3.split("action ")[1].split())
...
def test_splice_into_prompt():
    ...
    out = cond.splice_retrieved_into_prompt(base, "retrieved 0: state 1; action 2")
    assert out.endswith(anchor), "anchor must remain at the very end"
    assert "Demos: retrieved 0" in out
    # no anchor -> appended
    out2 = cond.splice_retrieved_into_prompt(
        "Task: x", "retrieved 0: state 1; action 2"
    )
    assert out2.endswith("retrieved 0: state 1; action 2")

...
class DualPartitionedGMMHead(GMMActionHead):
    ...
    def __init__(self, d_model, action_dim, k_agnostic, k_specific, embodiments,
                 chunk_len=1, min_std=1e-4, std_activation="softplus", low_noise_eval=True,
                 head_hidden_dim=None, head_n_layers=0, gate_n_layers=3, gate_hidden_dim=None,
                 d_model_a=None, d_model_s=None, share_agnostic=True):
        d_a = int(d_model_a) if d_model_a is not None else int(d_model)
        d_s = int(d_model_s) if d_model_s is not None else int(d_model)
        if int(k_agnostic) < 0 or int(k_specific) < 0 or int(k_agnostic) + int(k_specific) < 1:
            raise ValueError(f"need k_agnostic,k_specific >= 0 and sum >= 1 "
                             f"(got {k_agnostic}, {k_specific})")
        super().__init__(d_model=d_model, action_dim=action_dim,
                         num_modes=int(k_agnostic) + int(k_specific), min_std=min_std,
                         std_activation=std_activation, low_noise_eval=low_noise_eval,
                         chunk_len=chunk_len, head_hidden_dim=None, head_n_layers=0)
        ...
        self.A_head = per_emb(lambda: _ModeHead(d_a, C, Ka, D, head_hidden_dim, head_n_layers), agn_embs)
        self.S_head = per_emb(lambda: _ModeHead(d_s, C, Ks, D, head_hidden_dim, head_n_layers), self.embodiments)
        self.gate = per_emb(lambda: _GateHead(d_a + d_s, C, M, ghd, gate_n_layers), self.embodiments)
    ...
    def forward(self, a_top, s, emb_id=None):  # type: ignore[override]
        ...
        means = torch.cat([a_raw[..., 0, :], s_raw[..., 0, :]], dim=-2)   # (..., C, M, D)
        scales = torch.cat([a_raw[..., 1, :], s_raw[..., 1, :]], dim=-2)
        logits = pick(self.gate, emb_id)(torch.cat([a_top, s], dim=-1)).reshape(*lead, C, M)
        with torch.no_grad():
        ...
        means_flat = means.reshape(*lead, C, M * D)
        scales_flat = scales.reshape(*lead, C, M * D)
        raw = torch.cat([means_flat, scales_flat, logits], dim=-1).reshape(*lead, C * self.per_step)
        return raw

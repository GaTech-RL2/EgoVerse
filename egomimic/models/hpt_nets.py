...
class DINOv2(PolicyStem):
    ...
    def _ensure_runtime_attrs(self) -> None:
        # Lightning checkpoints pickle the whole model object, so loading an old
        # DINOv2 checkpoint does not re-run __init__. Attributes added after
        # training need safe defaults before train()/forward() touches them.
        if not hasattr(self, "feature_indices"):
            self.feature_indices = None
        if not hasattr(self, "lora_rank"):
            self.lora_rank = 0
        if not hasattr(self, "neck"):
            self.neck = None

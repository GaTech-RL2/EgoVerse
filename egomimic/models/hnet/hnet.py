...
class HNet(nn.Module):
    def __init__(self, stages: List[_BaseStage]):
        ...
        for i in range(len(stages) - 1):
            ...
            # ``ModuleList`` below is the sole owner of every stage.  Keep the
            # recursive runtime link as a plain Python attribute so PyTorch
            # does not serialize the same stage again through
            # ``stages.i.inner_stage``.
    ...
    def _load_from_state_dict(
        ...
        """Accept checkpoints written before stage registration was fixed.

        Those checkpoints contain every stage under its canonical
        ``stages.N`` path and again under one or more ``inner_stage`` aliases.
        Canonical values are sufficient, so discard only the redundant aliases
        before strict loading checks for unexpected keys.
        """
        stages_prefix = f"{prefix}stages."
        for key in tuple(state_dict):
            if key.startswith(stages_prefix) and ".inner_stage." in key:
                state_dict.pop(key)
...
def chunk_stats_from_aux(aux: List[dict]) -> dict:
    ...
    ``boundary_rate_0``, ...) plus aggregate ``boundary_rate`` and
    ``avg_chunk_len`` values averaged across chunkers. All values are Python
    ...
    out["avg_chunk_len"] = sum(lengths) / len(lengths)
    return out

...
def routing_output_from_probs(
    boundary_p: torch.Tensor,
) -> RoutingModuleOutput:
    """Build the standard deterministic routing result from ``P(boundary)``.

    The argmax intentionally selects the non-boundary class when
    ``boundary_p == 0.5``, preserving the existing router tie behaviour.
    """
    boundary_prob = torch.stack((1 - boundary_p, boundary_p), dim=-1)

...
class InterStreamComm(nn.Module):
    ...
    def __init__(
        self, d_model_q, num_heads, src_dims: List[int], rotary_emb_dim, dropout=0.0
    ):
        super().__init__()
        assert d_model_q % num_heads == 0
        self.d_model_q, self.num_heads, self.head_dim = (
            d_model_q,
            num_heads,
            d_model_q // num_heads,
        )
        ...
        self.k_proj = nn.ModuleList(
            [nn.Linear(dj, d_model_q, bias=True) for dj in src_dims]
        )
...
class MultiStreamBlock(nn.Module):
    ...
    def __init__(
        ...
        def _mk_intra(i):
            s = streams_cfg[i]
            return IntraStreamModule(
                s["d_model"], s["num_heads"], s["d_intermediate"], _rot(s), dropout
            )

        def _mk_inter(i, srcs):
            src_dims = [streams_cfg[j]["d_model"] for j in srcs]
            return InterStreamComm(
                streams_cfg[i]["d_model"],
                streams_cfg[i]["num_heads"],
                src_dims,
                _rot(streams_cfg[i]),
                dropout,
            )
        ...
        self.inter = nn.ModuleDict(
            {
                str(i): per_emb(
                    lambda i=i, srcs=srcs: _mk_inter(i, srcs), embs if i >= 1 else None
                )
    ...
    def forward(
        self, xs: List[torch.Tensor], mask, rope_positions, emb_id: Optional[str] = None
    ):
        xs = [
            pick(self.intra[i], emb_id)(xs[i], mask, rope_positions)
        ...
        for i, srcs in enumerate(self.adjacency):
            if srcs:
                ...
                xs[i] = inter(
                    post_intra[i], [post_intra[j] for j in srcs], mask, rope_positions
                )
        return xs
...
class MultiStreamComputeStage(_BaseStage):
    ...
    def __init__(
        ...
        if int(input_hidden_dim) != int(output_hidden_dim):
            raise ValueError(
                "MultiStreamComputeStage requires input==output hidden dim."
            )
    ...
    def _build_mask(self, T_total, cu_seqlens, time_pos, device):
        ...
        allow = (same_ep & time_ok) | torch.eye(
            T_total, dtype=torch.bool, device=device
        )
        return allow

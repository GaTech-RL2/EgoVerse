...
class DualStreamRouter(nn.Module):
    ...
    def __init__(
        ...
        n_layers=4,
        fusion="residual_mlp",
        router_pre_layout=None,
        router_pre_detach=False,
        hidden_mult=4.0,
    ):
        super().__init__()
        ...
        if self.fusion == "residual_mlp":
            ...
            self.mixer = build_residual_mixer(
                "mlp", d_router, n_layers=int(n_layers), hidden_mult=float(hidden_mult)
            )
            ...
            self.fuse = self._build_mlp(
                d_a + d_s, d_router, int(n_layers), int(d_router * hidden_mult)
            )
            for m in self.fuse:
            ...
            raise ValueError(
                f"router fusion must be 'residual_mlp'|'mlp', got {fusion!r}"
            )
...
class DualStreamChunkerStage(_BaseStage):
    def __init__(
        ...
        s_output_hidden_dim: Optional[
            int
        ] = None,  # apex_s (specific inner dim; dual_inner only)
        ...
        if (
            ...
            or up_interface != "duplicate"
            or router_interface != "cossim"
        ):
        ...
        if self.grab_prev_end:
            self.prev_end_combine_S = per_emb(
                lambda: _combine(d_s, s_target), self._embs
            )
            self.no_prev_S = per_emb(
                lambda: nn.Parameter(torch.randn(d_s) * 0.02), self._embs
            )
            self.proj_in_S = None
        else:
            self.proj_in_S = (
                per_emb(lambda: nn.Linear(d_s, s_target), self._embs)
                if d_s != s_target
                else None
            )
        self.proj_out_S = (
            per_emb(lambda: nn.Linear(apex_s, d_s), self._embs)
            if (self.dual_inner and apex_s != d_s)
            else None
        )
    ...
    def _combine_prev_end(self, chunked, x_stream, bmask, cu, combine_mlp, no_prev):
        ...
        prev_end = torch.where(
            is_start.unsqueeze(-1), no_prev.to(prev_end.dtype), prev_end
        )
    ...
    def forward(self, x, ctx: HNetContext) -> StreamBundle:  # type: ignore[override]
        ...
        if not ctx.packed:
            raise NotImplementedError(
                "DualStreamChunkerStage supports the PACKED path only."
            )
        A, S = x.A, x.S
        cu = ctx.cu_seqlens
        ...
        if self.grab_prev_end:
            A_in = self._combine_prev_end(
                A_ch, A, bmask, cu, self.prev_end_combine_A, self.no_prev_A
            )
            S_in = self._combine_prev_end(
                S_ch,
                S,
                bmask,
                cu,
            ...
            proj_in_S = (
                pick(self.proj_in_S, emb) if self.proj_in_S is not None else None
            )
            S_in = proj_in_S(S_ch) if proj_in_S is not None else S_ch

        if self.a_to_s_inject:
            inject_proj, inject_gate = (
                pick(self.inject_proj, emb),
                pick(self.inject_gate, emb),
            )
        ...
        if self.dual_inner:
            ...
            bundle_in = StreamBundle(
                A=A_in, S=S_in, T_total=N, cu_seqlens=next_cu, time_pos=ctp
            )
            ctx.cu_seqlens, ctx.max_seqlen = next_cu, next_max
            try:
                bundle_out = (
                    self.inner_stage(bundle_in, ctx)
            ...
            A_inner = (
                self.proj_out_A(bundle_out.A)
                if self.proj_out_A is not None
                else bundle_out.A
            )
            proj_out_S = (
                pick(self.proj_out_S, emb) if self.proj_out_S is not None else None
            )
            S_inner = (
                proj_out_S(bundle_out.S) if proj_out_S is not None else bundle_out.S
            )
            ...
            try:
                ...
                ctx.cu_seqlens, ctx.max_seqlen = saved_cu, saved_max
            A_inner = (
                self.proj_out_A(A_inner) if self.proj_out_A is not None else A_inner
            )
            A_dech = self.dechunk_A(A_inner, bmask, bprob, cu_seqlens=next_cu)

#!/bin/bash
# BC isolation pair on 1xH200: hnet_bc_big (old chunkchunk recipe, lr 1e-4, bs8)
# vs hnet_b4_bc_big (b8 encdec b4 recipe, lr 1.4e-4, bs66). Circle-only feed;
# same rolling-ckpt/resume/logging pattern as pace_b8_launch.sh.
...
if [ "$cell" = "hnet_b4_bc_big" ]; then
  CELLOPTS="++data.train_dataloader_params.pushshapes_sim.batch_size=66 ++data.train_dataloader_params.pushshapes_sim.num_workers=4 ++model.optimizer.lr=1.4e-4 ++norm_stats.precomputed_norm_path=$G/b8_norm_stats.json"
else
  CELLOPTS="++data.train_dataloader_params.pushshapes_sim.num_workers=4 ++model.optimizer.lr=1e-4"
fi
srun $PY -m egomimic.trainHydra --config-name=train_zarr_cartesian +experiment=indomain_c4/$cell $RES logger=csv callbacks=ckpt_ratio_loss_scheduler_700 \
  ...
  $DATA_OV $CELLOPTS launch_params.gpus_per_node=1 ++trainer.strategy=ddp_find_unused_parameters_true \

...
def run(args: argparse.Namespace) -> int:
    ...
    env_args: dict[str, Any] = {
        ...
        "collector": "mouse",
        "mode": args.mode,
        "solid_pusher": bool(args.solid_pusher),
    }
    ...
    env = get_env(args.sim_version)(
        ...
        image_size=args.image_size,
        seed=args.seed,
        solid_pusher=args.solid_pusher,
        socket_inside_friction_only=(args.pusher == "u_socket"),
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--solid-pusher",
        action="store_true",
        help="collect against the fixed physics: the pusher is stopped by walls"
        " and obstacles instead of passing through them. Recorded into"
        " env_args so replay picks the right physics later.",
    )
    p.add_argument(
        "--replay-init-file",
        default=None,

...
def load_sources(path: Path) -> list[SourceDemo]:
    ...
    for episode in episode_dirs(path):
        ...
        demos.append(
            SourceDemo(
                ...
                control_gap=(dict(init["control_gap"])
                             if "control_gap" in init else None),
                control_gap_mode=(str(init["control_gap_mode"])
                                  if "control_gap_mode" in init else None),
            )
        )
    if not demos:
    ...
    signatures = {
        (
            ...
            tuple(sorted(demo.control_gap.items())) if demo.control_gap else None,
            demo.control_gap_mode,
        )
        for demo in demos
    }
    if len(signatures) != 1:
...
def main() -> int:
    ...
    while total < args.target_total:
        ...
        print(
            ...
            f"layout_failures={result.layout_failures} "

...
srun $PY -m egomimic.trainHydra --config-name=train_zarr_cartesian +experiment=indomain_c4/$cell $RES logger=csv callbacks=checkpoints \
  ...
  ++trainer.precision=bf16 ++trainer.limit_train_batches=100 ++trainer.limit_val_batches=0 ++trainer.num_sanity_val_steps=0 \
  ++trainer.max_epochs=3000 ++trainer.min_epochs=3000 ++trainer.check_val_every_n_epoch=200 \

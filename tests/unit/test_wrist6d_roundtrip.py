...
def _assert_pose12_close(got, ref, atol):
    ...
    (immune to +-pi wrap and gimbal ambiguity)."""
...
def test_torch_ypr_matrix_matches_scipy_zyx():
    """The torch helpers and the numpy transforms must agree on what 'ypr'
...
def test_human_wristframe_6d_pipeline_round_trips_to_headframe(fix_left):
    ...
    # Independent head-frame ground truth. The Rz(180 deg) fix relabels the
...
def test_eva_wristframe_6d_pipeline_round_trips_to_camframe():
    ...
    # the rig calibration is ~6e-9 off orthonormal, hence 1e-7
    obs_ref = np.stack(
        [CartesianYPRToRot6D(action_key="k").transform({"k": g})["k"] for g in gt_obs]
    )
...
# ---------------------------------------------------------------------------
# The convention checker: does a zarr's stored wrist rotation match what its
# own keypoints produce under the pre-fix or the post-fix relabel?
# ---------------------------------------------------------------------------


def _synthetic_hand(rng, n_frames=5):
    """(n, 21, 3) keypoints whose wrist frame is well-conditioned."""
    kp = rng.normal(scale=0.02, size=(n_frames, 21, 3))
    kp[:, 0] = [0.0, 0.0, 0.0]  # wrist
    kp[:, 9] += [0.10, 0.0, 0.0]  # middle-finger base: forward axis
    kp[:, 5] += [0.03, 0.05, 0.0]  # thumb
    kp[:, 17] += [0.03, -0.05, 0.0]  # pinky
    return kp


def _poses_from(kp, relabel):
    from egomimic.scripts.mecka_process.mecka_to_zarr import _unrelabeled_hand_frame

    poses = np.zeros((len(kp), 7))
    for i, frame in enumerate(kp):
        q = R.from_matrix(_unrelabeled_hand_frame(frame) @ relabel).as_quat()
        poses[i, :3] = frame[0]
        poses[i, 3:] = [q[3], q[0], q[1], q[2]]  # stored WXYZ
    return poses


def _episode(tmp_path, left_relabel, right_relabel, name="ep"):
    import zarr

    from egomimic.scripts.mecka_process.mecka_to_zarr import (
        ROT_RELABEL_POST_FIX,
        ROT_RELABEL_PRE_FIX_LEFT,
    )

    relabels = {"pre_fix": ROT_RELABEL_PRE_FIX_LEFT, "post_fix": ROT_RELABEL_POST_FIX}
    rng = np.random.default_rng(0)
    g = zarr.open_group(str(tmp_path / f"{name}.zarr"), mode="w")
    for side, which in (("left", left_relabel), ("right", right_relabel)):
        kp = _synthetic_hand(rng)
        relabel = relabels[which] if isinstance(which, str) else which
        g[f"{side}.obs_keypoints"] = kp.reshape(len(kp), -1)
        g[f"{side}.obs_wrist_pose"] = _poses_from(kp, relabel)
    return g


@pytest.mark.parametrize("written", ["pre_fix", "post_fix"])
def test_left_wrist_convention_reads_the_data(tmp_path, written):
    from egomimic.scripts.mecka_process.mecka_to_zarr import left_wrist_convention

    g = _episode(tmp_path, left_relabel=written, right_relabel="post_fix")
    assert left_wrist_convention(g) == written


def test_left_wrist_convention_is_unknown_when_the_control_fails(tmp_path):
    from egomimic.scripts.mecka_process.mecka_to_zarr import left_wrist_convention

    # A third converter: the right hand matches neither relabel, so the
    # left-hand comparison is confounded (3 fold_clothes + 2 pick_place
    # episodes in the production set look like this).
    third = np.array([[0, 0, 1], [0, -1, 0], [1, 0, 0]])
    g = _episode(tmp_path, left_relabel=third, right_relabel=third)
    assert left_wrist_convention(g) == "unknown"


def test_left_wrist_convention_is_unknown_without_keypoints(tmp_path):
    import zarr
...
def _resolver(tmp_path, fix_enabled):
    from egomimic.rldb.embodiment.human import Human
    from egomimic.rldb.zarr.zarr_dataset_multi import S3EpisodeResolver

    return S3EpisodeResolver(
        tmp_path,
        key_map=Human.get_keymap(keymap_mode="cartesian"),
        transform_list=Human.get_transform_list(
            mode="cartesian_6d", stride=1, fix_left_wrist_convention=fix_enabled
        ),
    )


def test_resolver_rejects_a_double_flip(tmp_path):
    """Correcting data the converter already corrected is a fresh error."""
    _episode(tmp_path, left_relabel="post_fix", right_relabel="post_fix")
    datasets = {"ep": _FakeDataset(tmp_path / "ep.zarr")}
    with pytest.raises(ValueError, match="double-flip"):
        _resolver(tmp_path, True)._check_left_wrist_convention(datasets)


def test_resolver_only_warns_on_uncorrected_data(tmp_path, caplog):
    """Pre-fix data with the flag off is what every run did before this
    commit; it is wrong, but killing the generic human configs at startup is
    not this correction's business."""
    import logging

    _episode(tmp_path, left_relabel="pre_fix", right_relabel="post_fix")
    datasets = {"ep": _FakeDataset(tmp_path / "ep.zarr")}
    with caplog.at_level(logging.WARNING):
        _resolver(tmp_path, False)._check_left_wrist_convention(datasets)
    assert any("uncorrected frame" in r.message for r in caplog.records)


@pytest.mark.parametrize(
    "written,fix_enabled", [("pre_fix", True), ("post_fix", False)]
)
def test_resolver_accepts_a_flag_the_data_agrees_with(tmp_path, written, fix_enabled):
    _episode(tmp_path, left_relabel=written, right_relabel="post_fix")
    datasets = {"ep": _FakeDataset(tmp_path / "ep.zarr")}
    _resolver(tmp_path, fix_enabled)._check_left_wrist_convention(datasets)


def test_resolver_only_warns_on_an_unrecognised_convention(tmp_path, caplog):
    import logging

    third = np.array([[0, 0, 1], [0, -1, 0], [1, 0, 0]])
    _episode(tmp_path, left_relabel=third, right_relabel=third)
    datasets = {"ep": _FakeDataset(tmp_path / "ep.zarr")}
    with caplog.at_level(logging.WARNING):
        _resolver(tmp_path, True)._check_left_wrist_convention(datasets)
    assert any("wrist-convention" in r.message for r in caplog.records)

...
separate ``ZarrEpisode.read`` calls over six distinct keys; they are now one read of
the union span per key, sliced twice. Nothing the dataset emits may change.
...
def test_one_read_per_distinct_zarr_key(episode, monkeypatch):
    ...
    calls: list[dict] = []
    inner = ZarrEpisode.read
    # ZarrEpisode has __slots__, so the counter goes on the class
    monkeypatch.setattr(
        ZarrEpisode,
        "read",
        lambda self, spans: (calls.append(dict(spans)), inner(self, spans))[1],
    )
...
def test_union_span_covers_both_windows(episode):
    """A key read as both an action chunk and a proprio history spans both."""
    ds = ZarrDataset(episode, key_map=_keymap("keypoints_k4"), transform_list=None)
    windows = ds._read_windows(60)
    ...
    assert spans["left.obs_keypoints"] == (57, 60 + HORIZON)
    assert spans["obs_head_pose"] == (60, 61), "a key read once is not widened"

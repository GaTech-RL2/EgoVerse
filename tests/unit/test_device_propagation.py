...
``HPT.__init__`` picks a default device (cuda whenever one is visible) and
copies it into the ``HPTModel`` it builds. Lightning only learns the real
device later, and ``ModelWrapper.on_fit_start`` / ``on_validation_start`` write
it to the algo. If that write stops at the algo, ``HPTModel.compute_loss``
keeps building its loss accumulators on the construction-time device while
``HPT.compute_losses`` accumulates on the trainer's -- a CPU-tier run on a
machine with a visible GPU then dies with "Expected all tensors to be on the
same device".
"""

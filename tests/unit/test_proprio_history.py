...
def _dataset(tmp_path, *, proprio_history: int = 1, transforms=None, n: int = 1):
    for i in range(n):
        _write_varying_episode(tmp_path, seed=i)
    key_map = Human.get_keymap(keymap_mode="cartesian", proprio_history=proprio_history)
...
def test_flagship_defaults_to_k_one(compose_resolve):
    cfg = compose_resolve(RECIPE, [DATA])
    assert cfg.data.train_datasets.human_bimanual.resolver.key_map.proprio_history == 1
    stem = cfg.model.robomimic_model.stem_specs.human_bimanual.state_ee_pose
    assert stem.history_len == 1
    assert stem.history_dropout == 0.2  # inert at K = 1

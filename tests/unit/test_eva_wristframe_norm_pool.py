...
def test_eva_revert_for_rollout_recovers_camframe_ypr(mode):
    ...
    def run(m):
        ...
        # the ypr half of this comparison is exactly what a legacy checkpoint
        # feeds the rollout, which is the case allow_legacy_rotation covers
        for t in Eva.get_transform_list(m, allow_legacy_rotation=True):
            s = t.transform(s)
        batch = {

...
# documentation, not a guarantee; they must stay in step with the `train_viz`
# metric budget, which is what this file asserts.
...
def test_pipeline_per_variant(ladder, k):
    for variant, keymap, mode, pad in (
        ...
        assert res.transform_list.fix_mecka_left_wrist is True
...
@pytest.mark.parametrize("k", ARMS)
@pytest.mark.parametrize("variant", VARIANTS)
def test_metric_budget_tracks_the_documented_episode_counts(ladder, k, variant):
    """K = floor(5120 / episodes-in-split) keeps each head inside the 80x64 frame
    validation window after EvenStrideDataset subsampling. The train_viz counts
    are measured, so this also catches a header updated without its budget."""
    mfe = ladder[(k, variant)].metric_frames_per_episode
    assert mfe.train_viz == 5120 // TRAIN_EPISODES[k]
    assert mfe.valid == 5120 // SEEN_VAL_EPISODES
    assert mfe.train_viz * TRAIN_EPISODES[k] <= 5120
    assert mfe.valid * SEEN_VAL_EPISODES <= 5120

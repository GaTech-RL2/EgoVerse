...
def _release_key(name: str) -> str:
    """The module-tree name as google/paligemma-3b-pt-224 spells it."""
    for old, new in (
...
def test_a_real_paligemma_loads_on_this_transformers(tmp_path):
    """The miniature tree above is spelled the 5.x way; a real model is spelled
    whichever way the installed transformers spells it, which is how a 4.53
    venv came to fail 21 SigLIP tensors while the key tests all passed."""
    source = _tiny_paligemma()
    released = {
        _release_key(n): p.detach().clone().contiguous()
        for n, p in source.named_parameters()
    }
    target = _tiny_paligemma()
    model = nn.Module()
    model.paligemma_with_expert = nn.Module()
    model.paligemma_with_expert.paligemma = target
    pgi.load_paligemma_weights(model, _write_checkpoint(tmp_path, released))
    for name, param in target.named_parameters():
        assert torch.equal(param, dict(source.named_parameters())[name]), name


def test_a_nested_vision_tree_is_found():
    own = {"model.vision_tower.vision_model.post_layernorm.bias": None}
    key = "vision_tower.vision_model.post_layernorm.bias"
    assert (
        pgi._module_name(key, own)

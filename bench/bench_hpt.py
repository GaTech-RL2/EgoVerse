...
def run(args, variant: str):
    ...
    def grad_norm_hooks():
        ...
        ``once`` is today's wrapper: on_after_backward runs a full
        clip_grad_norm_(inf) over every parameter and reads it on the host.
        ``current`` adds the second pass on_before_optimizer_step used to run,
        which is what the README's numbers were taken with.
        """

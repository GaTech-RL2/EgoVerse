...
- **You are on a Linux SLURM cluster (Georgia Tech; working dir under `/coc/...`).** The login/dev node handles code editing, lint, type checks, and small read-only work. For GPU/CPU-intensive work you have two options:
  ...
  - **GPU memory ceiling here is 48 GB (a40 44 GB / l40s 48 GB) — no A100/H100/H200.** Full fp32-AdamW fine-tuning of pi0.5 (3.6 B params) needs ~57 GB and OOMs even at batch 1; use 8-bit AdamW (`uv pip install bitsandbytes`, `train_robotwin_ricl.py --adam8bit`) to fit (~22 GB + activations). The lab QOS allows ~1 GPU at a time (a 2nd request fails `QOSGrpGRES`); grab extra/idle GPUs via the preemptible `overcap` partition+account (`-p overcap -A overcap`). `l40s` is GRES-restricted for interactive `srun` — submit GPU work with `sbatch` (or `sbatch --wrap=...`), not `srun`.
...
- **The venv is `uv`-managed and has NO `pip`.** Install packages with `uv pip install <pkg>` (with the venv active). Bare `pip` silently resolves against a stray system Python 3.8 user-site — never use it; `python -m pip` fails (`No module named pip`).

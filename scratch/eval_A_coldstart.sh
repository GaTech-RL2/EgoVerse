...
run_eval () {
  ...
  $SRUN --jobid=$JID --overlap .venv/bin/python -m egomimic.trainHydra \
    --config-name=train_zarr_cartesian mode=eval name=pixelpol_A_cs description=$DESC \
    ...
    norm_stats.precomputed_norm_path="$NORM" logger=csv > /tmp/o_Acs_${DESC}.txt 2>&1
  echo "Acs_${DESC}_DONE rc=$?  $(grep -aoE 'emb15_sim_coverage[^0-9]+[0-9.]+' /tmp/o_Acs_${DESC}.txt | tail -1)"
}
run_eval cs_chunk1 1 1 2
run_eval cs_chunk4 1 4 5
run_eval cs_chunk8 1 8 9
echo ALL_ACS_DONE

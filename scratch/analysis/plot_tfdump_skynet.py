...
D = "/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/tf_dump_react_lowest"
...
fig.suptitle(
    "reactiveB_lowest (K=1 reactive, LR 1e-5) -- TEACHER-FORCED: GT (green) vs predicted (red) cursor path\n"
    "in-distribution diagnostic (correct obs fed each step, no rollout drift)",
    fontsize=13)
fig.tight_layout(rect=[0, 0, 1, 0.96])
out = f"{D}/WAR_reactiveB_lowest_tf_gt_vs_pred.png"
...
for i, f in enumerate(files):
    ...
    print(f"ep{i}: MAE={np.abs(pred-gt).mean():.2f}px RMSE={err.mean():.2f}px "
          f"first32={err[:32].mean():.2f} rest={err[32:].mean():.2f}")

...
D = "/coc/flash7/paphiwetsa3/projects/EgoVerse2/logs/tf_dump_chunkA_obsonly"
...
for i, f in enumerate(files):
    d = np.load(f)
    pred, gt = d["pred"], d["gt"]
    if pred.ndim == 3:           # safety: chunk (T,K,A) -> first action of each chunk
        pred = pred[:, 0]
    T = len(gt)
    ...
    ax.plot(pred[:, 0], pred[:, 1], "-", color="red", lw=1.5, alpha=0.85, label="pred (obs-only a0)")
    ...
    ax.scatter([pred[0, 0]], [pred[0, 1]], c="orange", marker="o", s=50, edgecolor="k", zorder=5)
    ax.scatter([pred[-1, 0]], [pred[-1, 1]], c="darkred", marker="X", s=70, edgecolor="k", zorder=5)
...
fig.suptitle(
    "chunk-A (teacher-forced, OBS-ONLY) -- GT demo (green) vs predicted (red)\n"
    "pred = first action of the obs-only 32-chunk from policy.generate (in-distribution)",
    fontsize=13)
fig.tight_layout(rect=[0, 0, 1, 0.96])
out = f"{D}/chunkA_tf_obsonly_gt_vs_pred.png"
...
for i, f in enumerate(files):
    ...
    err = np.sqrt(((pred - gt) ** 2).sum(1))
    print(f"ep{i}: MAE={np.abs(pred-gt).mean():.2f}px RMSE={err.mean():.2f}px shape_pred={d['pred'].shape} shape_gt={d['gt'].shape}")

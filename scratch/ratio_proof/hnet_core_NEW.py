...
class HNetCore(nn.Module):
    ...
    def __init__(
        ...
        # Per-forward collection sink for the chunker's auxiliary ratio loss.
        # ``_encode`` appends one already-weighted scalar per encoded window into
        # ``self._ratio_loss_parts``; ``forward`` resets it on entry and reduces
        # to ``self._last_ratio_loss`` (MEAN across windows — see ``forward``).
        # INERT unless the BC-RNN algo reads ``_last_ratio_loss`` (it only does
        # when ``collect_ratio_loss=True`` is configured); the chunker still
        # registers + we still compute aux every call, but the value is simply
        # ignored on the default path, so default behavior is unchanged.
        self._ratio_loss_parts = []
    ...
    def _encode(self, x_in: torch.Tensor) -> torch.Tensor:
        ...
        x = self.in_proj(x_in)
        ctx = self._new_ctx()
        out = self.hnet(x, ctx)  # (B, L, d_model)
        # Collect the chunker's auxiliary ratio loss for THIS window. The
        # ChunkerStage registered its ``bpred``/``target_ratio``/``weight`` into
        # ``ctx.aux`` during the forward above; ``ratio_loss_from_aux`` reduces
        # it to the paper's ratio loss with the per-stage ``weight``
        # (== ``self.ratio_loss_weight``) ALREADY baked in (it multiplies each
        # entry by ``entry["weight"]``). So the appended scalar is the
        # FULLY-WEIGHTED ratio loss; do NOT re-multiply by ratio_loss_weight
        # downstream. Always computed (cheap, keeps the chunker's aux graph
        # alive for grad flow when collected); only READ when the algo opts in.
        self._ratio_loss_parts.append(
            ratio_loss_from_aux(ctx.aux, device=out.device)
        )
        return out
    ...
    def forward(self, obs_emb: torch.Tensor, hidden: Optional[dict] = None):
        ...
        """
        # Reset the per-forward ratio-loss sink; each ``_encode`` below appends
        # one already-weighted scalar. We MEAN across windows (not sum) so the
        # magnitude is window-count-invariant — on the TRAINING path there is
        # exactly ONE window (T <= max_window), so the mean == that single
        # window's ratio loss; only the multi-window EVAL-OVERLAY path averages.
        # (forward_eval runs under no_grad, so its averaged value never reaches
        # the optimizer; the convention only matters for parity of the logged
        # number, and mean matches the per-window training value.)
        self._ratio_loss_parts = []
        T = obs_emb.shape[1]
        if T <= self.max_window:
            out = self._encode(obs_emb)
            self._last_ratio_loss = self._ratio_loss_parts[0]
        ...
        # MEAN the per-window ratio losses (see the note at the top of forward).
        self._last_ratio_loss = torch.stack(self._ratio_loss_parts).mean()
        return out, {"obs": last}

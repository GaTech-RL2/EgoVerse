...
run_eval () {
  ...
  $SRUN --jobid=$JID --overlap .venv/bin/python -m egomimic.trainHydra \
    --config-name=train_zarr_cartesian mode=eval name=pixelpol_A_csf description=$DESC \
    ...
    norm_stats.precomputed_norm_path="$NORM" logger=csv > /tmp/o_Acsf_${DESC}.txt 2>&1
  echo "Acsf_${DESC}_DONE rc=$?  $(grep -aoE 'emb15_sim_coverage[^0-9]+[0-9.]+' /tmp/o_Acsf_${DESC}.txt | tail -1)"
}

#!/bin/bash
# Iter 2: decouple_action_noise + attn-dropout (random per-batch, now also hits DiTAttention).
set -u
export COLUMNS=250
...
srun --jobid=$JID --export=ALL .venv/bin/python -m egomimic.trainHydra \
  ...
  name=dfot_cutaction_attndrop description=cutaction_attndrop_v1 mode=train \
  ...
  callbacks=ckpt_attn_dropout callbacks.model_checkpoint.every_n_epochs=30 \
  ...
  logger=csv_wandb > /tmp/o_train_attndrop.txt 2>&1
echo "TRAIN rc=$?  $(tail -2 /tmp/o_train_attndrop.txt)"
R=$(ls -dt logs/dfot_cutaction_attndrop/* 2>/dev/null | head -1)
ls -la $R/checkpoints/ 2>/dev/null
cp $R/checkpoints/last.ckpt external_ckpts/cutaction_attndrop_v1_last.ckpt 2>/dev/null && echo COPIED_last

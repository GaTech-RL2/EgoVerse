...
CKPT=$(ls -t logs/pixelpol_B/B_regress_v1*/checkpoints/*.ckpt 2>/dev/null | head -1)
NORM=$(ls -t logs/pixelpol_B/B_regress_v1*/norm_stats/norm_stats.json 2>/dev/null | head -1)
echo "ckpt=$CKPT"
echo "norm=$NORM"
...
run_eval () {
  ...
  $SRUN --jobid=$JID --overlap .venv/bin/python -m egomimic.trainHydra \
    --config-name=train_zarr_cartesian mode=eval name=pixelpol_B_eval description=$DESC \
    ...
    model=dfot_pushshapes_pixel_regress model.robomimic_model.inference_mode=pixel_regress \
    ...
    norm_stats.precomputed_norm_path="$NORM" logger=csv > /tmp/o_B_${DESC}.txt 2>&1
  echo "B_${DESC}_DONE rc=$?  $(grep -aoE 'emb15_sim_coverage[^0-9]+[0-9.]+' /tmp/o_B_${DESC}.txt | tail -1)"
}

run_eval chunk 1 8
run_eval recede 4 1
$SCANCEL $JID 2>/dev/null
echo "ALL_B_EVALS_DONE"

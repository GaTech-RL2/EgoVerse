...
JID=$1
run_eval () {
  local DESC=$1; local NCTX=$2; local COMMIT=$3; local NS=$4
  $SRUN --jobid=$JID --overlap .venv/bin/python -m egomimic.trainHydra \
    --config-name=train_zarr_cartesian mode=eval name=pixeldec_chunk description=$DESC \
    ...
    ++model.robomimic_model.sp_n_context=$NCTX ++model.robomimic_model.sp_commit=$COMMIT ++model.robomimic_model.sp_n_samples=$NS \
    ...
    norm_stats.precomputed_norm_path="$NORM" logger=csv > /tmp/o_DECchunk_${DESC}.txt 2>&1
  echo "DECchunk_${DESC}_DONE rc=$?  $(grep -aoE 'emb15_sim_coverage[^0-9]+[0-9.]+' /tmp/o_DECchunk_${DESC}.txt | tail -1)"
}
run_eval c8_s8 1 8 8
run_eval c4_s8 1 4 8
echo ALL_DECCHUNK_DONE
